...
pub struct Parser<'a> {
    ...
    pub(crate) errors: Vec<Box<dyn Error>>,
}
...
impl<'a> Parser<'a> {
    ...
    pub fn parse_statement(&mut self) -> Statement {
        ...
        match self.curr_token {
            Token::Keyword(Keyword::Constrain) => {
                return Statement::Constrain(ConstraintParser::parse_constrain_statement(self));
            }
            _ => {
                let expr_stmt = self.parse_expression_statement();
                return Statement::Expression(Box::new(expr_stmt));
            }
        };
    }
    ...
    pub(crate) fn parse_expression(&mut self, precedence: Precedence) -> Option<Expression> {
            ...
            && (precedence < Precedence::from(&self.peek_token))
        {
            match self.infix_fn() {
                None => {
                    println!("No infix function found for {}", &self.curr_token); // XXX: This is a user error, so that's why we don't panic( Add better error msg). Unless I forgot to implement an infix for a operator
...
///XXX(low) : Move all of these tests into their new modules
#[cfg(test)]
mod test {
    use super::*;
    use librasac_ast::{
        CallExpression, FunctionDefinition, FunctionLiteral, Ident, IfExpression, InfixExpression,
        Literal, PrefixExpression, Type,
    };
    #[test]
    fn test_basic_let() {
