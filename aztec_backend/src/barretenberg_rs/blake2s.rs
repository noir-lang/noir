...
pub fn hash_to_field(input : &[u8]) -> FieldElement {
    ...
    let input_ptr = barretenberg.allocate(input); // 0..32
    ...
    barretenberg.call_multiple("blake2s_to_field", vec![&input_ptr, &data_len, &result_ptr]);

    barretenberg.free(input_ptr);

    let result_bytes = barretenberg.slice_memory(0, 32);
...
fn basic_interop() {
    ...
    for test in tests {
        ...
        let got = hash_to_field(&test.input);
