...
impl Poseidon2 {
    ...
    fn field_from_hex(hex: &str) -> FieldElement {
        let bigint = BigUint::from_str_radix(hex.strip_prefix("0x").unwrap(), 16).unwrap();
    ...
    fn add_round_constants(&self, state: &mut [FieldElement], round: usize) {
        for (state_element, constant_element) in state.iter_mut().zip(self.round_constant[round]) {
    ...
    fn internal_m_multiplication(&self, input: &mut [FieldElement]) {
        ...
        for (index, i) in input.iter_mut().enumerate() {
            *i = *i * self.internal_matrix_diagonal[index];
            *i += sum;
        }
    }
    ...
    pub(crate) fn permutation(
        ...
        if len != self.t {
            return Err(BlackBoxResolutionError::Failed(
                acir::BlackBoxFunc::Poseidon2Permutation,
                format!("Expected {} values but encountered {}", self.t, len),
            ));
        }
        ...
        // Internal rounds
        let p_end = rf_first + self.rounds_p;
        for r in rf_first..p_end {
            state[0] += self.round_constant[r as usize][0];
            state[0] = Self::single_box(state[0]);
            self.internal_m_multiplication(&mut state);
        }

        // Remaining external rounds
        let num_rounds = self.rounds_f + self.rounds_p;
