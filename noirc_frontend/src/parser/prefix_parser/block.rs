...
impl BlockParser {    
    ...
    pub(crate) fn parse_block_expression(parser: &mut Parser) -> Result<BlockExpression, ParserError> {
        ...
        if parser.curr_token != Token::LeftBrace {
            return Err(ParserErrorKind::UnstructuredError{message : format!("Expected a {{ to start the block expression"), span : parser.curr_token.into_span()}.into_err(parser.file_id));
        }
...
#[cfg(test)]
mod test {
    use crate::{BlockExpression, ExpressionKind, parser::test_parse, token::Token};
