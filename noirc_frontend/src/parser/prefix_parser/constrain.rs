...
impl ConstrainParser {
    ...
    pub(crate) fn parse_statement(parser: &mut Parser) -> Result<ConstrainStatement, ParserError> {
        ...
        if infix.operator.contents == BinaryOpKind::Assign {
            ...
            return Err(ParserErrorKind::UnstructuredError{message, span: infix.operator.span()}.into_err(parser.file_id))
        }
        
        if disallowed_operators().contains(&infix.operator.contents) {
            let message = format!("Cannot use the {} operator in a constraint statement.",infix.operator.contents.as_string());
            return Err(ParserErrorKind::UnstructuredError{message, span: expr.span}.into_err(parser.file_id))
        }
...
mod test {
    ...
    use super::{ConstrainParser, disallowed_operators};
    ...
    fn valid_syntax() {
        ...
        assert_eq!(start.token(), &Token::Keyword(crate::token::Keyword::Constrain));
    ...
    fn invalid_assign() {
        ...
        let mut parser = test_parse(SRC);
        let _err = ConstrainParser::parse_statement(&mut parser).unwrap();
    }
}