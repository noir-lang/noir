...
// We could actually use a vector of tuples (K,V) and have a enum marker to show scope beginning
/*
enum Item {
    Marker,
    Symbol(K, V)
...
impl<K : std::hash::Hash + Eq, V> ScopeTree<K, V> {
    ...
    pub fn last_scope(&mut self) -> &mut Scope<K,V> {
        self.0.last_mut().expect("Compiler Error: Tried to fetch the last scope, however no Scopes are present")
    }
