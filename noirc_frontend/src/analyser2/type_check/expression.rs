use crate::{analyser2::errors::TypeErrorKind, ast::{Expression, ExpressionKind, Type, Literal, InfixExpression, ArraySize}};
use super::*;
    ...
    pub fn type_check_expr(&mut self,expr :  &mut Expression) -> Result<Type, AnalyserError> {
        match &mut expr.kind {
            ...
            ExpressionKind::Call(path, call_expr) => {
                ...
                let func = func.expect(&format!("Compiler Error: Could not find a function named {} , under the path {:?}", &call_expr.func_name.last_segment().0.contents, path));

                let (parameters, return_type) = (func.parameters.clone(), func.return_type.clone());
