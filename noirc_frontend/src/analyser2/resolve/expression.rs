    ...
    pub(crate) fn resolve_expr(&mut self, expr : &Expression) -> bool{
        match &expr.kind{
            ...
            ExpressionKind::Call(path,call_expr) => {
                ...
                let func = self.find_function(&call_expr.func_name);
                let span = call_expr.func_name.span();
                let name = call_expr.func_name.last_segment();
                let name_str = name.0.contents.clone();
                
                let func = match func {
                    None => {
                        let err = ResolverErrorKind::Unresolved{span, symbol_type : "function".to_owned(), symbol : name_str}.into_err(self.file_id);
                        self.push_err(err);
                        return false
                    },
                    Some(func) => func,
                ...
                let param_len = func.parameters.len();
                let argument_len = call_expr.arguments.len();

                if param_len != argument_len {
                    let message = format!("Function {} expected {} number of arguments, but got {}", &name_str, param_len, argument_len);
                    let err = AnalyserError::from_ident(self.file_id,message, &name);
                    self.push_err(err);
                }
