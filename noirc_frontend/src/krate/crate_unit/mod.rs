...
impl<Module> CrateUnit<Module> {
    ...
    pub fn debug_virtual_paths(&self) {
        let v : Vec<_> = self.virtual_path_to_module.keys().collect();
        dbg!(v);
    }
    ...
    pub fn module_ids(&self) -> Vec<ModID> {
        self.virtual_path_to_module.values().copied().collect()
    }
    ...
    pub fn insert_module(&mut self, file_path : PathBuf, module : Module) -> ModID{

        let relative_path = pathdiff::diff_paths(&file_path, self.root_dir.to_path_buf()).unwrap();
        ...
        let vp = VirtualPath::from_relative_path(relative_path);
        let old_value = self.virtual_path_to_module.insert(vp, mod_id);
        assert!(old_value.is_none(), "{:?}", old_value);
