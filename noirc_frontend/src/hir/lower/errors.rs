...
pub enum ResolverError {
    ...
    DuplicateDefinition { first_ident : IdentId, second_ident: IdentId},
    ...
    pub fn into_diagnostic(self, interner : &NodeInterner) -> Diagnostic {
        match self {
            ...
            ResolverError::VariableNotDeclared { name, span } => {
                ...
                Diagnostic::simple_error(format!("cannot find `{}` in this scope ", name), format!("not found in this scope"), span)
            },
            ResolverError::PathIsNotIdent { span } => {
                
                Diagnostic::simple_error(format!("cannot use path as an identifier"), String::new(), span)
            },
            ResolverError::PathUnresolved { span, name } => {
                
                Diagnostic::simple_error(format!("could not resolve path : {}", name), String::new(), span)
            },
            ResolverError::Expected { span, expected, got } => {
                
                Diagnostic::simple_error(format!("expected {} got {}", expected, got), String::new(), span)
            },
        }
    }
}