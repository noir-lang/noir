...
impl<'a> ModCollector<'a> {
    ...
    pub fn collect_defs(&mut self, context : &mut Context) {
        ...
        for import in self.ast.imports.clone() {
            self.def_collector.collected_imports.push(
                ImportDirective{ module_id : self.module_id,path : import.path, alias :import.alias}
            );
        }
    ...
    fn parse_module_declaration(&mut self, context : &mut Context, mod_name : &str) {
        let new_file_id = context.file_manager().resolve_path(self.file_id, mod_name).unwrap();
    ...
    pub fn push_child_module(&mut self, mod_name : &str, file_id : FileId) -> LocalModuleId {
        ...
        modules[self.module_id.0].children.insert(mod_name.to_owned(), LocalModuleId(module_id));
        ...
        let mod_id = ModuleId {krate : self.def_collector.def_map.krate, local_id : LocalModuleId(module_id)}.into();
