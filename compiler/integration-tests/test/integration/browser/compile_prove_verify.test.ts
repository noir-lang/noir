...
test_cases.forEach((testInfo) => {
  ...
  const mochaTest = new Mocha.Test(
    ...
    async () => {
      ...
      const noir_source = await getFile(
      ...
      const prover_toml = await getFile(
      ...
      const noirProgram = new NoirProgram(
        {
          bytecode: compile_output.circuit,
          abi: compile_output.abi,
        },
      ...
      await noirProgram.init();

      // JS Proving

      const proofWithPublicInputs =
        await noirProgram.generateFinalProof(inputs);

      // JS verification

      const verified = await noirProgram.verifyFinalProof(
      ...
      // const compiled_contract = await getFile(
      //   `${base_relative_path}/${testInfo.compiled}`,
      // );
      // const deploy_information = await getFile(
      ...
      // const { proof, publicInputs } = separatePublicInputsFromProof(
      //   proofWithPublicInputs,
      //   testInfo.numPublicInputs,
      // );
