...
describe("compile fuzz test", () => {
    for (let i = 0; i < iterations; i++) {
        it("should error compile() gracefully", async () => {
            ...
            await newABICoder();
            await initACVM();
            const fuzzedNoirSource = getRandomString(Math.floor(Math.random() * Math.pow(10, i)));
            ...
            try {
                ...
                expect(e.toString().includes("RuntimeError: unreachable executed")).to.be.true;
                errorOccurred = true;
            }
            expect(errorOccurred, "Compilation should have failed with the fuzzed input").to.be.true;
        });
    }
});
