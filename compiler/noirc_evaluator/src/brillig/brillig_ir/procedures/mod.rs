...
mod tests {
    ...
    /// 1. the concrete peak documented in `design/brillig/scratch_space.md` stays accurate, and
    /// 2. it stays comfortably below `MAX_SCRATCH_SPACE`, the hard ceiling the allocator enforces.
    ///
    /// If a procedure change moves the peak, update both the number here and the design doc.
    #[test]
    fn peak_scratch_demand_across_procedures() {
        ...
        let peak_address = all.into_iter().map(max_direct_address).max().unwrap();
