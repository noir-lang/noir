...
impl<F: AcirField + DebugToString, Registers: RegisterAllocator> BrilligContext<F, Registers> {
    /// Generates a check-last (do-while style) loop that saves one opcode per iteration
    /// compared to a check-first loop by eliminating the unconditional jump back to the loop head.
    ///
