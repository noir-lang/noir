...
pub(crate) enum BrilligParameter {
    ...
    /// A list parameter or return value. Holds the type of a list item.
    /// Only known-length lists can be passed to brillig entry points, so the size is available as well.
    List(Vec<BrilligParameter>, usize),
}
