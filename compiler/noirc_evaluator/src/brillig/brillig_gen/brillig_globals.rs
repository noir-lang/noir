...
use crate::brillig::{allocate_value_with_type, brillig_gen::BrilligBlock, brillig_ir::BrilligContext, BrilligBinaryOp, DataFlowGraph, GlobalSpace, Instruction, ReservedRegisters, SingleAddrVariable, Type, Value};
...
impl<'global> BrilligGlobals<'global> {
                ...
                Value::Instruction { instruction, .. } => {
                    ...
                    match &instruction {
                        Instruction::MakeArray { elements: array, typ } => {
                            let new_variable = allocate_value_with_type(self.brillig_context, typ.clone());
                            ...
                            // Write the items
                            let items_pointer = self.brillig_context
    ...
    fn initialize_constant_array(
            ...
            && item_types.iter().all(|typ| matches!(typ, Type::Numeric(_)))
        {
            ...
            self.initialize_constant_array_runtime(
                item_types, first_item, item_count, pointer
            );
        } else {
    ...
    fn initialize_constant_array_runtime(
        ...
        for subitem_id in item_to_repeat.into_iter() {
            subitem_to_repeat_variables.push(*self.brillig_globals.get(&subitem_id).unwrap_or_else(|| panic!("ICE: ValueId {subitem_id} is not available")));
        }

        // Initialize loop bound with the array length
        let end_pointer_variable = self.brillig_context
    ...
    fn initialize_constant_array_comptime(
        ...
        for (element_idx, element_id) in data.iter().enumerate() {
            let element_variable = *self.brillig_globals.get(&element_id).unwrap_or_else(|| panic!("ICE: ValueId {element_id} is not available"));
