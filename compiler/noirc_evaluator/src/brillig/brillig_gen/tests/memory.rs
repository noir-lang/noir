use crate::{
    assert_artifact_snapshot, brillig::brillig_gen::tests::ssa_to_brillig_artifacts,
    ssa::ir::map::Id,
};

// Tests array element access by index code-gen for Brillig.
#[test]
fn brillig_array_get() {
    let src = "
    brillig(inline) fn foo f0 {
      b0(v0: u32):
        v1 = make_array [u32 10, u32 20, u32 30] : [u32; 3]
        v2 = array_get v1, index v0 -> u32
        return v2
    }
    ";

    let brillig = ssa_to_brillig_artifacts(src);
    let foo = &brillig.ssa_function_to_brillig[&Id::test_new(0)];
    assert_artifact_snapshot!(foo, @r"
    fn foo
     0: call 0
     1: sp[1] = @1
     2: @3 = const u32 2048
     3: @1 = u32 add @1, @3
     4: sp[3] = const u32 10
     5: sp[4] = const u32 20
     6: sp[5] = const u32 30
     7: sp[6] = @1
     8: sp[7] = const u32 4
     9: @1 = u32 add @1, sp[7]
    10: sp[6] = indirect const u32 1
    11: sp[7] = u32 add sp[6], @2
    12: sp[8] = sp[7]
    13: store sp[3] at sp[8]
    14: sp[8] = u32 add sp[8], @2
    15: store sp[4] at sp[8]
    16: sp[8] = u32 add sp[8], @2
    17: store sp[5] at sp[8]
    18: sp[4] = u32 add sp[6], @2
    19: sp[5] = u32 add sp[4], sp[2]
    20: sp[3] = load sp[5]
    21: sp[2] = sp[3]
    22: return
    ");
}

// Tests setting an array element and retrieving it
#[test]
fn brillig_array_set() {
    let src = "
    brillig(inline) fn foo f0 {
      b0(v0: u32):
        v1 = make_array [u32 10, u32 20, u32 30] : [u32; 3]
        v2 = array_set v1, index v0, value u32 99
        v3 = array_get v2, index v0 -> u32
        return v3
    }
    ";

    let brillig = ssa_to_brillig_artifacts(src);
    let foo = &brillig.ssa_function_to_brillig[&Id::test_new(0)];
    assert_artifact_snapshot!(foo, @r"
    fn foo
     0: call 0
     1: sp[1] = @1
     2: @3 = const u32 2048
     3: @1 = u32 add @1, @3
     4: sp[3] = const u32 10
     5: sp[4] = const u32 20
     6: sp[5] = const u32 30
     7: sp[6] = @1
     8: sp[7] = const u32 4
     9: @1 = u32 add @1, sp[7]
    10: sp[6] = indirect const u32 1
    11: sp[7] = u32 add sp[6], @2
    12: sp[8] = sp[7]
    13: store sp[3] at sp[8]
    14: sp[8] = u32 add sp[8], @2
    15: store sp[4] at sp[8]
    16: sp[8] = u32 add sp[8], @2
    17: store sp[5] at sp[8]
    18: sp[3] = const u32 99
    19: @3 = sp[6]
    20: @4 = const u32 4
    21: call 0
    22: sp[4] = @5
    23: sp[5] = u32 add sp[4], @2
    24: sp[7] = u32 add sp[5], sp[2]
    25: store sp[3] at sp[7]
    26: sp[5] = u32 add sp[4], @2
    27: sp[6] = u32 add sp[5], sp[2]
    28: sp[3] = load sp[6]
    29: sp[2] = sp[3]
    30: return
    ");
}

// Tests array operations with reference counting inc_rc
#[test]
fn brillig_array_with_rc_ops() {
    let src = "
    brillig(inline) fn foo f0 {
      b0():
        v0 = make_array [u32 10, u32 20, u32 30] : [u32; 3]
        inc_rc v0
        v1 = array_set v0, index u32 0, value u32 99
        v2 = array_get v1, index u32 0 -> u32
        return v2
    }
    ";

    let brillig = ssa_to_brillig_artifacts(src);
    let foo = &brillig.ssa_function_to_brillig[&Id::test_new(0)];
    assert_artifact_snapshot!(foo, @r"
    fn foo
     0: call 0
     1: sp[1] = @1
     2: @3 = const u32 2048
     3: @1 = u32 add @1, @3
     4: sp[2] = const u32 10
     5: sp[3] = const u32 20
     6: sp[4] = const u32 30
     7: sp[5] = @1
     8: sp[6] = const u32 4
     9: @1 = u32 add @1, sp[6]
    10: sp[5] = indirect const u32 1
    11: sp[6] = u32 add sp[5], @2
    12: sp[7] = sp[6]
    13: store sp[2] at sp[7]
    14: sp[7] = u32 add sp[7], @2
    15: store sp[3] at sp[7]
    16: sp[7] = u32 add sp[7], @2
    17: store sp[4] at sp[7]
    18: sp[2] = load sp[5]
    19: sp[2] = u32 add sp[2], @2
    20: store sp[2] at sp[5]
    21: sp[2] = const u32 0
    22: sp[3] = const u32 99
    23: @3 = sp[5]
    24: @4 = const u32 4
    25: call 0
    26: sp[4] = @5
    27: sp[6] = u32 add sp[4], sp[2]
    28: store sp[3] at sp[6]
    29: sp[5] = u32 add sp[4], sp[2]
    30: sp[3] = load sp[5]
    31: sp[2] = sp[3]
    32: return
    ");
}
