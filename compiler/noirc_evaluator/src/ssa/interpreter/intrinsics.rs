            ...
            Intrinsic::BlackBox(black_box_func) => match black_box_func {
                ...
                acvm::acir::BlackBoxFunc::MultiScalarMul => {
                    ...
                    for (i, v) in input_points.elements.borrow().iter().enumerate() {
                        let field_val = v.as_field().ok_or(
                        ...
                        points.push(field_val);
                        // After every y coordinate (odd index), derive is_infinite
                        if i % 2 == 1 {
                            let x = points[points.len() - 2];
                            let is_infinite: FieldElement = if x.is_zero() && field_val.is_zero() {
                                FieldElement::one()
                            } else {
                                FieldElement::zero()
                            };
                            points.push(is_infinite);
                        }
                    }
                    let scalars =
                    ...
                    let (x, y, _is_infinite) = result.map_err(Self::convert_error)?;
                ...
                acvm::acir::BlackBoxFunc::EmbeddedCurveAdd => {
                    check_argument_count(args, 5, intrinsic)?;
                    let solver = bn254_blackbox_solver::Bn254BlackBoxSolver;
                    let lhs_x = self.lookup_field(args[0], "call EmbeddedCurveAdd BlackBox")?;
                    let lhs_y = self.lookup_field(args[1], "call EmbeddedCurveAdd BlackBox")?;
                    let lhs_infinite: FieldElement = if lhs_x.is_zero() && lhs_y.is_zero() {
                        FieldElement::one()
                    } else {
                        FieldElement::zero()
                    };
                    let rhs_x = self.lookup_field(args[2], "call EmbeddedCurveAdd BlackBox")?;
                    let rhs_y = self.lookup_field(args[3], "call EmbeddedCurveAdd BlackBox")?;
                    let rhs_infinite: FieldElement = if rhs_x.is_zero() && rhs_y.is_zero() {
                        FieldElement::one()
                    } else {
                        FieldElement::zero()
                    };
...
fn new_embedded_curve_point(x: FieldElement, y: FieldElement) -> IResult<Value> {
    ...
    Ok(Value::array(
        vec![x, y],
        vec![Type::Numeric(NumericType::NativeField), Type::Numeric(NumericType::NativeField)],
    ))
}
