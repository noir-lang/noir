...
fn create_apply_function(
    ...
    ssa.add_fn(|id| {
        ...
        for (index, (function_id, runtime)) in function_ids.iter().enumerate() {
            let is_second_last = index == function_ids.len() - 2;
            // If the last function to dispatch has a mismatched runtime, this function
            // is now the last function to dispatch.
            let is_last = if is_second_last
                && function_ids[index + 1].1.is_acir()
                && caller_runtime.is_brillig()
            {
                true
            } else {
                index == function_ids.len() - 1
            };
            ...
            // As we have special handling for the last function to dispatch in the case of differing
            // runtimes between the closure variant and the caller, we must exit the loop manually as to maintain a well-formed SSA.
            if is_last {
                break;
            }
        }
...
mod tests {
    ...
    fn acir_variant_in_brillig_last_function_to_dispatch() {
       ...
       "#;
        ...
        // It ensures our function would still fail if there was an attempt to call f3 with the final constrain
        // that exists for the last function to dispatch.
        // If `Field 3` were ever to be passed to `apply` as an attempt to call f3, the function will fail.
        assert_ssa_snapshot!(ssa, @r"
        brillig(inline) fn main f0 {
          b0(v0: Field, v1: Field, v2: Field):
            v4 = allocate -> &mut Field
    ...
    fn acir_variant_in_brillig_not_last_to_dispatch() {
      ...
      brillig(inline) fn main f0 {
        ...
        b1():
          jmp b3(f1, f2)
        b2():
          v9, v10 = call f3() -> (function, function)
          jmp b3(v9, v10)
