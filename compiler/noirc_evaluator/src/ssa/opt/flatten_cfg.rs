            ...
            match instruction {
                ...
                Instruction::Call { func, mut arguments } => match self.inserter.function.dfg[func]
                {
                    ...
                    Value::Intrinsic(Intrinsic::BlackBox(BlackBoxFunc::EmbeddedCurveAdd)) => {
                        ...
                        // x, y of LHS
                        arguments[0] =
                            self.mul_by_condition(arguments[0], casted_condition, call_stack);
                        arguments[1] =
                            self.mul_by_condition(arguments[1], casted_condition, call_stack);
                        // is_infinity of LHS
                        arguments[2] = self.var_or_one(arguments[2], condition, call_stack);
                        // x, y of RHS
                        arguments[3] =
                            self.mul_by_condition(arguments[3], casted_condition, call_stack);
                        arguments[4] =
                            self.mul_by_condition(arguments[4], casted_condition, call_stack);
                        // is_infinity of RHS
        ...
        if let Some((array, typ)) = &self.inserter.function.dfg.get_array_constant(argument) {
            ...
            for (i, value) in array.clone().iter().enumerate() {
                if i % 3 != 2 {
                    ...
                    // set x, y to zero
                    array_with_predicate.push_back(self.mul_by_condition(
                        *value,
                        casted_predicate,
                        call_stack,
                    ));
