...
//!
//! In the case of lists, they are represented as Brillig vectors as [RC, Size, Capacity, ...items],
//! thus the items pointer instead starts at three rather than one. So for a list
...
/// Given an array or list value and a constant index, returns an offset (shifted) index.
fn compute_offset_index(
    context: &mut SimpleOptimizationContext,
    array_or_list: ValueId,
    index: ValueId,
) -> Option<ValueId> {
    let constant_index = context.dfg.get_numeric_constant(index)?;
    let offset = context.dfg.array_offset(array_or_list, index);
...
mod tests {
    ...
    #[test]
    fn offset_list_array_get_constant_index() {
        let src = "
        brillig(inline) fn main f0 {
          b0(v0: [Field]):
            v2 = array_get v0, index u32 0 -> Field
    ...
    #[test]
    fn offset_list_array_set_constant_index() {
        let src = "
        brillig(inline) fn main f0 {
          b0(v0: [Field]):
            v2 = array_set v0, index u32 0, value Field 2
