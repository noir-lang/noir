...
///
/// A `JmpIf` may have both `then_destination` and `else_destination` pointing at
/// the same successor, in which case both argument vectors are returned so the
/// caller can wire each edge. Panics if the given block does not terminate in a
/// Jmp or JmpIf.
