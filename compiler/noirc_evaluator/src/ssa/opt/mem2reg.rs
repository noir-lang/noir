...
fn collect_eligible_variables_and_def_sites(
    ...
    // stored* on some path. Such variables are already filtered out in `variables`
...
/// path, and so can be safely promoted to registers.
///
/// Promotion replaces a load with the value that reaches it and merges divergent values with a
/// block parameter. If no store happens before the load, it would be invalid.
/// Note that a well-formed SSA should always *store before use*.
///
...
/// which only forgoes the optimization.
