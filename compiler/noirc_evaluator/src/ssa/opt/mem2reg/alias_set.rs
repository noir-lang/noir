...
impl AliasSet {
    ...
    pub(super) fn known(value: Value) -> AliasSet {
        let mut aliases = BTreeSet::new();
        aliases.insert(value);
        Self { aliases: Some(aliases) }
    }

    pub(super) fn known_multiple(values: BTreeSet<Value>) -> AliasSet {
    ...
    pub(super) fn single_alias(&self) -> Option<Value> {
        self.aliases
            .as_ref()
    ...
    pub(super) fn insert(&mut self, new_alias: Value) {
    ...
    pub(super) fn any(&self, f: impl FnMut(Value) -> bool) -> Option<bool> {
        self.aliases.as_ref().map(|aliases| aliases.iter().copied().any(f))
    }

    pub(super) fn for_each(&self, mut f: impl FnMut(Value)) {
    ...
    pub(super) fn first(&self) -> Option<Value> {
