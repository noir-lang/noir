...
/// this complementarity — most of `simplify::IfElse` only considers
...
/// would not (see #8301 for the both-zero variant; the both-one variant has the
/// same structure).
///
/// The check accepts:
///   - a constant pair that is not `(1, 1)` — `(0, 0)` is valid (both branches
///     disabled), `(0, 1)` and `(1, 0)` are valid (one side fires);
///   - a non-constant side paired with a constant zero on the other side —
///     trivially disjoint;
///   - two non-constant sides whose AND-decompositions differ by exactly one
///     leaf each, where those two leaves are negations of each other.
fn validate_if_else_conditions(
    ...
    if let (Some(t), Some(e)) = (then_const, else_const) {
        assert!(
            !(t.is_one() && e.is_one()),
            "IfElse: then_condition and else_condition are both constant 1; \
    ...
    // A constant one on either side requires the other side to be constant zero,
    // which the previous check would have accepted.
    ...
    // Both sides are non-constant. Decompose each into the multiset of leaves
    // of its AND-tree and require the symmetric difference to be exactly two
    // leaves that negate each other.
    ...
    let both_unique = then_only.len() == 1 && else_only.len() == 1;
    if both_unique && is_negation_of(then_only[0], else_only[0], dfg) {
        return;
    }
...
fn is_negation_of(a: ValueId, b: ValueId, dfg: &DataFlowGraph) -> bool {
...
mod tests {
    ...
    #[test]
    fn if_else_allows_shared_and_factor_with_negated_inner() {
        let src = "
        acir(inline) impure fn main f0 {
          b0(v0: u1, v1: u1):
            v2 = and v0, v1
    ...
    #[test]
    fn if_else_allows_constant_zero_pair() {
        let src = "
        acir(inline) impure fn main f0 {
          b0():
    ...
    #[test]
    fn if_else_allows_else_constant_zero() {
        let src = "
        acir(inline) impure fn main f0 {
          b0(v0: u1):
    ...
    #[test]
    #[should_panic(expected = "IfElse: then_condition and else_condition are both constant 1")]
    ...
    #[test]
    #[should_panic(expected = "is not the structural negation of")]
    fn if_else_disallows_shared_factor_without_negated_inner() {
        let src = "
        acir(inline) impure fn main f0 {
