            ...
            SsaReport::Bug(bug) => {
                ...
                let (secondary_message, call_stack) = match bug {
                    ...
                    InternalBug::UncheckedBrilligCall { call_stack } => {
                        ("There is no assert checking this brillig call's inputs against its return values in some way. This should be done to prevent potential soundness vulnerabilities".to_string(), call_stack)
                    }
