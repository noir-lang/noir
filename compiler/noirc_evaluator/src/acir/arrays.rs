...
impl Context<'_> {
    ...
    pub(super) fn init_element_type_sizes_array(
        ...
        dfg: &DataFlowGraph,
        additional_capacity: SemanticLength,
    ) -> Result<BlockId, RuntimeError> {
        ...
        if self.initialized_arrays.contains(&element_type_sizes) && additional_capacity.0 == 0 {
        ...
        match array_acir_value {
            ...
            AcirValue::DynamicArray(inner) if additional_capacity.0 == 0 => {
    ...
    pub(super) fn get_flattened_index(
        ...
        if let Some(step_size) = array_has_constant_element_size(array_typ) {
            ...
            let element_type_sizes = self.init_element_type_sizes_array(
                array_typ,
                array_id,
                None,
                dfg,
