...
fn indexing_array_with_non_u32_produces_an_error() {
    ...
    fn main() {
        ...
        let array = [1, 2, 3];
        let _ = array[index];
                      ^^^^^ Indexing arrays and lists must be done with `u32`, not `Field`
    }
    "#;
    check_errors(src);
}
...
#[test]
fn cannot_determine_list_type() {
    let src = r#"
    fn main() {
        let _ = &[];
                ^^^ Type annotation needed
                ~~~ Could not determine the type of the list
    }
    "#;
    check_errors(src);
}
...
#[test]
fn array_with_nested_list() {
    let src = r#"
    fn main () {
        let _: [[[Field]; 1]; 1] = [[&[0]]];
               ^^^^^^^^^^^^^^^^^ Nested lists, i.e. lists within an array or list, are not supported
