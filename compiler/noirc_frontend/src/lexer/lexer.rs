...
mod tests {
    ...
    fn test_ensures_attribute() {
        ...
        assert_eq!(
            token.token(),
            &Token::Attribute(Attribute::Secondary(crate::token::SecondaryAttribute::Ensures(
                Tokens(
                    expected_body
                        .iter()
                        .map(|x| SpannedToken::new(x.clone(), Span::empty(0)))
                        .collect()
                )
            )))
        );
    ...
    fn test_requires_attribute() {
        ...
        assert_eq!(
            token.token(),
            &Token::Attribute(Attribute::Secondary(crate::token::SecondaryAttribute::Requires(
                Tokens(
                    expected_body
                        .iter()
                        .map(|x| SpannedToken::new(x.clone(), Span::empty(0)))
