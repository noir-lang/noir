...
impl NodeInterner {
    ...
    /// Whether the type is a `#[transparent]` single-field struct, which serializes to the ABI
    /// as its inner field rather than as a wrapping struct.
    pub fn is_transparent(&self, type_id: TypeId) -> bool {
        self.type_attributes
            .get(&type_id)
            .is_some_and(|attributes| attributes.iter().any(|attr| attr.kind.is_transparent()))
    }
