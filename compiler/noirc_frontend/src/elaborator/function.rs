...
impl Elaborator<'_> {
    ...
    /// Returns the FuncIds of top-level free functions that have no comptime
    /// (meta) attribute. Their meta resolution can be deferred until after
    /// attributes have run so that their signatures may reference items
    /// generated by attribute expansion.
    #[tracing::instrument(level = "trace", skip_all)]
    pub(super) fn deferable_top_level_function_ids(
        function_sets: &[UnresolvedFunctions],
    ) -> HashSet<FuncId> {
        function_sets
            .iter()
            .flat_map(|function_set| &function_set.functions)
