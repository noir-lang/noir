...
use crate::{
    ...
    hir::{
        def_collector::dc_crate::{
            CollectedItems, ImplMap, UnresolvedFunctions, UnresolvedTraitImpl,
        },
...
impl Elaborator<'_> {
    ...
    /// Returns the FuncIds of functions whose meta resolution can be deferred
    /// until after attributes have run, so their signatures may reference items
    /// generated by attribute expansion. This covers top-level free functions,
    /// impl methods, and trait impl methods that don't carry a comptime
    /// attribute themselves, plus all trait method declarations (which are
    /// always registered with deferred metas — see [Elaborator::resolve_trait_methods]).
    ///
    /// Returns an empty set for the stdlib crate. The stdlib doesn't generate
    /// types via comptime attributes, but its globals and functions exercise
    /// stdlib-only paths (e.g. the array→vector coercion in
    /// [crate::hir_def::types::Type::try_array_to_vector_coercion]) that read
    /// `function_meta` directly with only `&NodeInterner`. Those paths can't
    /// lazy-resolve, so we keep the old eager flow inside the stdlib.
    #[tracing::instrument(level = "trace", skip_all)]
    pub(super) fn deferable_function_ids(&self, items: &CollectedItems) -> HashSet<FuncId> {
        if self.crate_id.is_stdlib() {
            return HashSet::new();
        }

        let top_level = items.functions.iter().flat_map(|fs| &fs.functions);
        let impl_methods =
            items.impls.values().flat_map(|sets| sets.iter()).flat_map(|(_, _, fs)| &fs.functions);
        let trait_impl_methods = items.trait_impls.iter().flat_map(|ti| &ti.methods.functions);

        let attributed_or_unattributed = top_level
            .chain(impl_methods)
            .chain(trait_impl_methods)
            .filter_map(|(_, func_id, func)| {
                let has_meta_attribute = func
                    .secondary_attributes()
                    .iter()
                    .any(|attr| matches!(attr.kind, SecondaryAttributeKind::Meta(_)));
                (!has_meta_attribute).then_some(*func_id)
            });

        let trait_methods = items.traits.values().flat_map(|t| t.method_ids.values().copied());

        attributed_or_unattributed.chain(trait_methods).collect()
    }
