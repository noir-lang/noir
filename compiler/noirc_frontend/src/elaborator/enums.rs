...
impl Elaborator<'_> {
    /// Collects all enum definitions and either resolves their variants eagerly
    /// (in the stdlib) or registers them for lazy resolution (in user crates,
    /// so that variant parameter types can reference types generated by
    /// comptime attribute expansion).
    #[tracing::instrument(level = "trace", skip_all)]
    pub(super) fn collect_enum_definitions(&mut self, enums: &BTreeMap<TypeId, UnresolvedEnum>) {
        if self.crate_id.is_stdlib() {
            for (type_id, typ) in enums {
                self.resolve_one_enum_variants(*type_id, typ.module_id, &typ.enum_def);
            }
        } else {
            for (type_id, typ) in enums {
    ...
    /// Resolves a single enum's variants and registers each variant's
    /// constructor (function or global). Used by both the eager stdlib path
    /// and the lazy/deferred path for non-stdlib crates.
    fn resolve_one_enum_variants(
        &mut self,
        type_id: TypeId,
        ...
        self.add_existing_generics(&enum_def.generics, &datatype_ref.generics);
        ...
        let location = enum_def.location;
        let unresolved =
