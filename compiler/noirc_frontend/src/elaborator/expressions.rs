    ...
    pub(super) fn elaborate_expression(&mut self, expr: Expression) -> (ExprId, Type) {
        let (hir_expr, typ) = match expr.kind {
            ...
            ExpressionKind::Variable(variable, generics) => {
                let generics = generics.map(|option_inner| option_inner.into_iter().map(|generic| self.resolve_type(generic)).collect());
    ...
    fn elaborate_method_call(
        ...
        match self.lookup_method(&object_type, method_name, span) {
            Some(method_ref) => {
                ...
                let generics = method_call.generics.map(|option_inner| option_inner.into_iter().map(|generic| self.resolve_type(generic)).collect());
