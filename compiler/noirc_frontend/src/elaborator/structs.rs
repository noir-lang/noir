...
impl Elaborator<'_> {
    ...
    /// (in the stdlib, since stdlib globals construct stdlib structs) or
    /// registers them for lazy resolution (in user crates, so that struct
    /// fields can reference types generated by comptime attribute expansion).
    ///
    ...
    pub(super) fn check_for_nested_vectors_in(
        ...
        for id in structs.keys() {
            ...
            if struct_type.borrow().generics.is_empty() {
                // `get_fields` returns `None` if the struct's fields are still
                // unresolved (e.g. its `UnresolvedStructFields` entry was held
                // back because it's in the outer call's pending set). Skip
                // those — the outer call will run this check itself.
                let Some(fields) = struct_type.borrow().get_fields(&[]) else { continue };
