...
impl<'context> Elaborator<'context> {
    ...
    pub(crate) fn elaborate_items(&mut self, mut items: CollectedItems) {
        ...
        // Resolve every function meta except for top-level free functions that
        // don't carry a comptime attribute. Those are kept deferred so that
        // their signatures may mention items generated by attribute expansion
        // (e.g. a struct produced by a `#[macro]` on another function).
        // Attributed functions, impl methods and trait impl methods are still
        // resolved here so that:
        //   * a global's RHS can call user-defined methods (method lookup
        //     reads each candidate's meta directly),
