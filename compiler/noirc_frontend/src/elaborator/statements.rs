...
impl Elaborator<'_> {
    ...
    pub(super) fn elaborate_for(&mut self, for_loop: ForLoopStatement) -> (HirStatement, Type) {
        ...
        let old_loop = self.item.body.enter_loop(true);
        self.push_scope();
        ...
        self.pop_scope();
        let _ = self.item.body.exit_loop(old_loop);
    ...
    pub(super) fn elaborate_loop(&mut self, loop_: LoopStatement) -> (HirStatement, Type) {
        ...
        let old_loop = self.item.body.enter_loop(false);
        self.push_scope();
        ...
        let last_loop = self.item.body.exit_loop(old_loop);
    ...
    fn elaborate_jump(&mut self, is_break: bool, location: Location) -> (HirStatement, Type) {
        ...
        if let Some(current_loop) = self.item.body.current_loop_mut() {
            if is_break {
    ...
    fn fresh_reference_binding(
        ...
        let counter = self.item.body.next_lvalue_index_counter();
