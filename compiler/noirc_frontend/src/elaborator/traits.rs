...
impl Elaborator<'_> {
    ...
    pub(crate) fn collect_trait_methods(
        ...
        for (trait_id, unresolved_trait) in traits {
            self.with_trait_scope(*trait_id, unresolved_trait.module_id, |this| {
                this.item
                    .generics
                    .set_params(this.interner.get_trait(*trait_id).all_generics.clone());
    ...
    pub(super) fn remove_trait_constraints_from_scope<'a>(
        ...
        for constraint in constraints {
            ...
            self.item.generics.remove_bounds_on_trait(constraint.trait_bound.trait_id);
        }
    ...
    fn resolve_trait_constraint_and_add_to_scope(
        ...
        self.item.generics.add_bound(constraint.clone());
        Some(constraint)
    }
    ...
    /// through unrelated variables leave it unresolvable. Failing that, a fresh variable per use
    /// at least keeps separate uses of the trait from sharing one.
    fn instantiate_implicit_associated_types(
        ...
        for named_type in named.iter_mut() {
            ...
            let typ = match in_scope {
                ...
                None => Type::TypeVariable(TypeVariable::unbound(
    ...
    /// excluding the constraints themselves. Each result is also pushed to
    ...
    fn register_trait_function(
        ...
        self.deferred.function_metas.register(
            ...
            UnresolvedFunctionMeta {
                func: function,
                local_module,
                impl_context: ImplContext::in_trait(trait_id, self_type),
                outer_generics: self.item.generics.params().to_vec(),
                extra_trait_constraints,
            },
        );
