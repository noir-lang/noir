...
impl Elaborator<'_> {
    ...
    fn instantiate_implicit_associated_types(
        ...
        let in_scope = self
            ...
            .trait_bounds
            .iter()
            .find(|bound| bound.trait_bound.trait_id == trait_id && bound.typ == *object);
    ...
    /// [`ItemContext::trait_bounds`] so `T::Assoc` syntax can reach it while the item is
    /// elaborated.
    #[tracing::instrument(level = "trace", skip_all)]
    pub(super) fn implied_where_clause_constraints(
        ...
        while let Some(constraint) = queue.pop() {
                ...
                self.trait_where_clause_implications(&constraint.typ, &constraint.trait_bound)
            {
                ...
                self.item.trait_bounds.push(implication.clone());
