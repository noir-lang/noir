...
use noirc_errors::{
    CustomDiagnostic, Location, Span, function_locations::FunctionLocations,
...
pub fn function_locations_for_diagnostics(
    diagnostics: &[CustomDiagnostic],
    parsed_files: &ParsedFiles,
) -> FunctionLocations {
    ...
    let mut function_name = FunctionLocations::new();
    let include_comptime_items = true;
    for file_id in file_ids {
        if let Some((parsed_file, _)) = parsed_files.get(&file_id) {
            let mut visitor = FunctionLocationsCollector::new(
                &mut function_name,
                file_id,
                include_comptime_items,
            );
...
pub fn function_locations_in_parsed_module(
    ...
    let mut visitor =
        FunctionLocationsCollector::new(&mut function_locations, file_id, include_comptime_items);
...
impl<'a> FunctionLocationsCollector<'a> {
    fn new(
        function_locations: &'a mut FunctionLocations,
        file_id: FileId,
        include_comptime_items: bool,
    ) -> Self {
        Self { function_locations, file_id, include_comptime_items, path: Vec::new() }
    }
...
impl Visitor for FunctionLocationsCollector<'_> {
