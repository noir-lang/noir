...
pub fn monomorphize(
    ...
    let mut monomorphizer = Monomorphizer::new(interner, files, force_unconstrained);
...
impl<'interner> Monomorphizer<'interner> {
    ...
    pub fn into_program(self) -> Program {
        ...
        Program::new(functions, self.return_location, globals)
            .handle_ownership()
    ...
    fn function_reference(
        ...
        // except for the `print` oracle, which accepts references so values can be
        // printed for debugging; a reference is rendered as `<<ref>>` by the executor.
