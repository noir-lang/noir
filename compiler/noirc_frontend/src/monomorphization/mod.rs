...
impl<'interner> Monomorphizer<'interner> {
    ...
    pub(crate) fn expr(&mut self, expr: ExprId) -> Result<ast::Expression, MonomorphizationError> {
        ...
        let expr = match self.interner.expression(&expr) {
            ...
            HirExpression::Literal(HirLiteral::Unit) => Literal(Unit),
    ...
    fn function_reference(
        ...
        // except for debug oracles, which are generated by the debugger and may use references
        // TODO: The debugger should not generate oracles with references in signature.
        let mut function_type = Cow::Borrowed(typ);
        if self.function_is_oracle(func_id)
            && !self.function_is_debug_oracle(func_id)
            && let Type::Function(args, ret, env, unconstrained) = typ
        {
            if self.function_is_print_oracle(func_id) {
                // The `print` oracle receives a reference-free value: the compiler loads any
                // reference in the printed value before the call (see `dereference_print_value`),
                // so the oracle is monomorphized with the dereferenced, reference-free signature
                // and no longer carries a reference parameter.
                self.check_return_type_returned_from_oracle(ret.as_ref(), location)?;
                let args = try_vecmap(args, |arg| Self::strip_references(arg, location))?;
                function_type =
                    Cow::Owned(Type::Function(args, ret.clone(), env.clone(), *unconstrained));
            } else {
                self.check_oracle_signature(args, ret.as_ref(), location)?;
            }
        }

        let typ = Self::convert_type(&function_type, location)?;
        ...
        if is_closure_type {
            ...
            let env = ast::Expression::ExtractTupleField(ident_clone, 0);
            let function = ast::Expression::ExtractTupleField(Box::new(ident_expression), 1);
    ...
    fn convert_type_helper(
        ...
        Ok(match typ.as_ref() {
            ...
            HirType::FmtString(size, fields) => {
                ...
                let fields =
                    Rc::new(Self::convert_type_helper(fields.as_ref(), location, seen_types)?);
            ...
            HirType::Array(element, length) => {
                ...
                let element =
                    Rc::new(Self::convert_type_helper(element.as_ref(), location, seen_types)?);
            ...
            HirType::Function(args, ret, env, _unconstrained) => {
                ...
                let ret = Rc::new(Self::convert_type_helper(ret, location, seen_types)?);
                let env = Rc::new(Self::convert_type_helper(env, location, seen_types)?);

                let make_function = |is_unconstrained,
                                     args,
                                     ret: Rc<ast::Type>,
                                     env: Rc<ast::Type>| {
                    use ast::Type::*;
                    match env.as_ref() {
                        Unit => Function(args, ret, env, is_unconstrained),
                        Tuple(_) => Tuple(vec![
                            env.as_ref().clone(),
                            Function(args, ret, env, is_unconstrained),
                        ]),
                        _ => {
                            unreachable!(
            ...
            HirType::Reference(element, mutable) => {
                ...
                ast::Type::Reference(Rc::new(element), *mutable)
            }
    ...
    fn is_closure_type(&self, t: &ast::Type) -> bool {
        if let ast::Type::Function(_, _, env, _) = t {
            return matches!(env.as_ref(), ast::Type::Tuple(_captures));
        ...
        unreachable!("ICE - Expected a closure type, found {t}");
    }
    ...
    fn check_return_type_crossing_runtime_boundaries(
        ...
        if return_type.contains_function() {
            return Err(MonomorphizationError::UnconstrainedFunctionReturnToConstrained {
                typ: return_type.to_string(),
                location,
            });
        }
    ...
    fn strip_references(typ: &Type, location: Location) -> Result<Type, MonomorphizationError> {
        ...
        match typ.follow_bindings() {
            Type::Reference(inner, _) => Self::strip_references(&inner, location),
            Type::Tuple(fields) => {
                let fields = try_vecmap(&fields, |field| Self::strip_references(field, location))?;
                Ok(Type::Tuple(fields))
            }
            Type::DataType(def, args) if def.borrow().get_fields(&args).is_some() => {
                let fields = def.borrow().get_fields(&args).unwrap();
                let fields =
                    try_vecmap(&fields, |(_, field, _)| Self::strip_references(field, location))?;
                Ok(Type::Tuple(fields))
            }
    ...
    /// Build an expression that loads every reference within `value` so the result is
    /// reference-free, mirroring [`Self::strip_references`].
    fn dereference_print_value(
        value: ast::Expression,
        typ: &Type,
        location: Location,
    ) -> Result<ast::Expression, MonomorphizationError> {
        ...
        match typ.follow_bindings() {
            Type::Reference(inner, _) => {
                let result_type = Self::convert_type(&inner, location)?;
                let dereferenced = ast::Expression::Unary(ast::Unary {
                ...
                Self::dereference_print_value(dereferenced, &inner, location)
            }
            Type::Tuple(fields) => Self::dereference_print_tuple(value, &fields, location),
            Type::DataType(def, args) if def.borrow().get_fields(&args).is_some() => {
                let fields = def.borrow().get_fields(&args).unwrap();
                let fields = vecmap(fields, |(_, field, _)| field);
                Self::dereference_print_tuple(value, &fields, location)
            }
            Type::Array(element, length) => {
                let length = length
                    .evaluate_to_u32(location)
                    .map_err(|err| MonomorphizationError::UnknownArrayLength { location, err })?;
                let element_type = Self::convert_type(&element, location)?;
                let contents = try_vecmap(0..length, |index| {
                    let index = ast::Expression::Literal(ast::Literal::Integer(
                        FieldElement::from(index),
                        ast::Type::Field,
                        location,
                    ));
                    let element_value = ast::Expression::Index(ast::Index {
                        collection: Box::new(value.clone()),
                        index: Box::new(index),
                        element_type: element_type.clone(),
                        location,
                    });
                    Self::dereference_print_value(element_value, &element, location)
                })?;
                let stripped =
                    Self::convert_type(&Self::strip_references(&element, location)?, location)?;
    ...
    /// Helper for [`Self::dereference_print_value`]
    /// Rebuild a tuple value by dereferencing each field with [`Self::dereference_print_value`].
    fn dereference_print_tuple(
        value: ast::Expression,
        fields: &[Type],
        location: Location,
    ) -> Result<ast::Expression, MonomorphizationError> {
        let elements = try_vecmap(fields.iter().enumerate(), |(index, field)| {
            let element = ast::Expression::ExtractTupleField(Box::new(value.clone()), index);
            Self::dereference_print_value(element, field, location)
        })?;
    ...
    fn closure_with_shared_env(
        ...
        let unconstrained_fn_typ =
            ast::Type::Function(parameter_types, Rc::new(ret_type), env_typ.clone(), true);

        // Build closure tuples: (env, fn) for each variant
        let constrained_closure_typ =
            ast::Type::Tuple(vec![env_typ.as_ref().clone(), constrained_fn_typ.clone()]);
        let unconstrained_closure_typ =
            ast::Type::Tuple(vec![env_typ.as_ref().clone(), unconstrained_fn_typ.clone()]);
    ...
    /// The `print` oracle (`#[oracle(print)]`) is exempt from the reference-parameter rule: the
    /// compiler loads any reference in the printed value before the call, so the oracle is
    /// monomorphized with a reference-free signature (see [`Self::dereference_print_value`]).
