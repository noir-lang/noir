...
use crate::field_element::{FieldElementExt, field_helpers};
...
impl Monomorphizer<'_> {
    ...
    fn patch_debug_var_assign(
        ...
        let source_var_id = source_var_id.absolute_value().try_into_u128().unwrap_or(0).into();
    ...
    fn patch_debug_member_assign(
        ...
        for i in 0..arity {
            ...
            {
                let index = fe_i.absolute_value().try_into_u128().unwrap_or(0);
                if fe_i.is_negative() {
                    ...
                    let integer = HirLiteral::Integer(SignedField::positive(field_helpers::field_from_u64(field_index as u64)));
                    let index_id = self.interner.push_expr(HirExpression::Literal(integer));
                    self.interner.push_expr_type(index_id, crate::Type::U256);
    ...
    fn intern_var_id(&mut self, var_id: DebugVarId, location: &Location) -> ExprId {
        let value = SignedField::positive(field_helpers::field_from_u32(var_id.0));
        let var_id_literal = HirLiteral::Integer(value);
        let expr_id = self.interner.push_expr(HirExpression::Literal(var_id_literal));
        self.interner.push_expr_type(expr_id, crate::Type::U256);
