...
pub enum InterpreterError {
    ...
    /// An error from running the code a macro call expanded to. It wraps the underlying error so
    /// the diagnostic also points at the macro call which generated that code.
    ErrorInMacroCall {
...
impl InterpreterError {
    ...
    pub fn location(&self) -> Location {
        match self {
            ...
            InterpreterError::ErrorInMacroCall { error, .. } => error.location(),
...
impl<'a> From<&'a InterpreterError> for CustomDiagnostic {
    fn from(error: &'a InterpreterError) -> Self {
        match error {
            ...
            InterpreterError::ErrorInMacroCall { error, macro_name, location } => {
                let mut diagnostic = CustomDiagnostic::from(&**error);
                diagnostic.add_secondary(
                    format!("While evaluating the code generated by `{macro_name}!`"),
                    *location,
                );
                diagnostic
            }
