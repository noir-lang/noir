...
pub fn resolve_path(
    ...
    if let Some(krate) = module_id.krate {
        let allow_referencing_contracts =
            allow_referencing_contracts(def_maps, Some(krate), module_id.local_id);
        ...
        Err(PathResolutionError::DummyCrateId(module_id, path))
    }
}
