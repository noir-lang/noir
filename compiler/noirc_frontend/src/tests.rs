...
// TODO: WIP comptime UHashMap test
#[test]
fn comptime_uhashmap_of_slices() {
    ...
    pub fn example_umap() -> UHashMap<u32, [u32]> {
        let _table = &[];
        let _len = 0;
    ...
    impl<K, V> Eq for UHashMap<K, V> {
        fn eq(self, _other: UHashMap<K, V>) -> bool { true }
    }

    fn main() {
        comptime let x = {
            let map = example_umap();
            map.eq(map)
        };
        assert(x);
    }
