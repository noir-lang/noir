...
use acvm::AcirField;
use acvm::acir::circuit::{AcirOpcodeLocation, ErrorSelector, Program, display_program};
use clap::Args;
...
/// Builds per-circuit opcode annotations (see [acir_opcode_annotations]) for every ACIR
/// function of a program, or `None` when source locations were not requested.
...
///
/// When `annotations` are present (see [acir_opcode_annotations]), each annotated
