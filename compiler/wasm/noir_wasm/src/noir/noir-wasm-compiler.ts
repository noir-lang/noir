...
import { initializeResolver } from './noir-source-resolver.shim';
...
export class NoirWasmContractCompiler {
  ...
  private constructor(entrypoint: NoirPackage, dependencyManager: NoirDependencyManager, wasmCompiler: any) {
   ...
   */
  public static new(projectPath: string, wasmCompiler: any) {
    if (!isAbsolute(projectPath)) {
      throw new Error('projectPath must be an absolute path');
    }

    const noirPackage = NoirPackage.open(projectPath, fs);

    const dependencyManager = new NoirDependencyManager(
      [
        new LocalDependencyResolver(fs),
        new GithubCodeArchiveDependencyResolver(fs),
  ...
  #resolveFile = (path: string) => {
    try {
      ...
      const data = this.#fm.readFileSync(libFile ?? path, 'utf-8') as string;
      return data;
    } catch (err) {
      return '';
    }
