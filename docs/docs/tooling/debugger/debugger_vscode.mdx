...
Name of the workspace package to debug. Defaults to the workspace's default package.
...
Thus `nargo dap` ships with a _preflight check_ mode. If flag `--preflight-check` and the rest of the `--preflight-*` flags are provided, Nargo will run the same initialization routine (type-check the project, locate the function to debug and read its inputs) except it will not start the DAP server.
...
| `--preflight-package <PREFLIGHT_PACKAGE>`               | Name of the workspace package to debug for preflight check.|
| `--preflight-prover-name <PREFLIGHT_PROVER_NAME>`       | Name of prover file to use for preflight check             |
| `--preflight-test-name <PREFLIGHT_TEST_NAME>`           | Name of the test function to debug for preflight check.    |
| `-h, --help`                                            | Print help.                                                |
