...
The _Variables_ section lists the locals of your program. At this point in execution it only holds the arguments of `main`, populated from your project's `Prover.toml` file, but as we step through the code it will get populated by `result`, `digest`, etc.
...
That covers most of the current debugger functionalities. Check out [the reference](../../tooling/debugger/debugger_vscode.mdx) for more details on how to configure the debugger.
