...
Both `print` and `println` are generic functions which can work on integers, fields, strings, and even structs or expressions. Note however, that lists are currently unsupported. For example:
