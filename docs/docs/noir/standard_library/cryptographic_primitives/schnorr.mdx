...
See schnorr::verify_signature_array for a version that works directly on arrays.
...
## schnorr::verify_signature_array

Verifier for Schnorr signatures over the embedded curve (for BN254 it is Grumpkin)
where the message is an array. 

#include_code schnorr_verify_array noir_stdlib/src/schnorr.nr rust
