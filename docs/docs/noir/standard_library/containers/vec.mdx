...
A vector is a collection type similar to Rust's `Vec<T>` type. In Noir, it is a convenient way to use lists as mutable arrays.
...
Creates a vector containing each element from a given list. Mutations to the resulting vector will not affect the original list.
...
let vector_from_list = Vec::from_list(list);
assert(vector_from_list.len() == 3);
```
...
```rust
let vector: Vec<Field> = Vec::from_list(&[10, 20, 30]);
...
```rust
let mut vector = Vec::from_list(&[10, 20]);
...
```rust
let mut vector = Vec::from_list(&[10, 30]);
vector.insert(1, 20);
assert(vector.get(1) == 20);
```
...
```rust
let mut vector = Vec::from_list(&[10, 20, 30]);
