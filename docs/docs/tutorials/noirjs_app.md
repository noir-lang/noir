...
We'll also need nargo installed, see the Noir [installation guide](../getting_started/noir_installation.md) for details.
...
```bash
yarn add @noir-lang/noir_js@1.0.0-beta.21 @aztec/bb.js@5.0.0-nightly.20260324 buffer vite vite-plugin-node-polyfills
```
...
`noir_js` and `bb.js` must be compatible: each `noir_js` release expects a matching Barretenberg version, so the versions above are pinned to a tested, working pair. A mismatched `bb.js` will fail to generate proofs, so if you bump one, bump the other to match.
