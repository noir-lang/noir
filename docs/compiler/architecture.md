...
The `tooling/fuzzer` crate contains utilities to generate random `InputMap` according to a circuit ABI
and bytecode. It is used by `nargo test` to generate input for `#[test]` methods that have parameters.

### Greybox Fuzzer

The `tooling/greybox_fuzzer` crate implements the `noir fuzz` machinery, which is an end-to-end solution
...
The `tooling/ssa_fuzzer` crate focuses on crafting SSA with arithmetic and logical operations,
...
The `tooling/ast_fuzzer` crate generates random monomorphized AST programs and performs comparative
