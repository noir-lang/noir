...
We'll also need version 1.0.0-beta.21 nargo installed, see the Noir [installation guide](../getting_started/noir_installation.md) for details.
...
```bash
yarn add @noir-lang/noir_js@1.0.0-beta.21 @aztec/bb.js@5.0.0-nightly.20260324 buffer vite vite-plugin-node-polyfills@0.26.0
```
...
In this guide, we will install versions pinned to 1.0.0-beta.21. These work with Barretenberg version 5.0.0-nightly.20260324, so we are using that one version too. Feel free to try with older or later versions, though!
...
All you need is a `main.nr` and a `Nargo.toml` file. You can follow the [noirup](../getting_started/noir_installation.md) installation and just run `noirup -v 1.0.0-beta.21`, or just create them by hand:
