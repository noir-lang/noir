...
:::info[Keccak proof]
We used `--oracle_hash keccak` to instruct bb to generate proof with `keccak` hash function, as that is more efficient in Solidity and our generated Solidity verifier expects a `keccak` proof. 
:::

To verify with Remix, you can pass the proof bytes as a hex string and the public inputs as a `bytes32[]` to the `verify` function.
...
Since we used `--output_format bytes_and_fields` flag, you will have a `target/public_inputs_fields.json` file containing an array of public inputs in hex format, which can be directly pasted into Remix.
...
Passing only two inputs will result in an error such as `PublicInputsLengthWrong()`.
