...
pub fn handle_sub_op(
    ...
    let negated_right = match &right {
        Object::Null => return Err(RuntimeErrorKind::UnstructuredError{span : Default::default(), message : format!("cannot do an operation with the null object")}),
        ...
        Object::Integer(_) => {
            ...
            if left_int.is_none() {
                return Err(RuntimeErrorKind::UnstructuredError{span : Default::default(), message : format!("rhs is an integer, however the lhs is not")})
            } else {
                return Ok(Object::Integer(left_int.unwrap().sub(right, env, evaluator)?));
            }
        }
        x => return Err(RuntimeErrorKind::UnsupportedOp{span : Default::default(), op : "sub".to_owned(), first_type : left.r#type().to_owned(), second_type :right.r#type().to_owned()}),
    };
