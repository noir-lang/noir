 ...
 */
template <typename Builder, typename Note> class UTXOStateVar : public StateVar<Builder> {
  public:
    ...
    UTXOStateVar(FunctionExecutionContext<Builder>* exec_ctx, std::string const& state_var_name)
        : StateVar<Builder>(exec_ctx, state_var_name){};

    // Instantiate a var nested within a container:
    UTXOStateVar(FunctionExecutionContext<Builder>* exec_ctx,
