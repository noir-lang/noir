...
namespace waffle
{
    ...
    inline plonk_linear_terms compute_linear_terms(const transcript::Transcript& transcript, const barretenberg::fr& l_1)
    {
        ...
        for (size_t i = 0; i < program_settings::program_width; ++i)
        {
            barretenberg::fr coset_generator = (i == 0) ? barretenberg::fr::one() : barretenberg::fr::coset_generator(i - 1);
        ...
        {
            barretenberg::fr permutation_evaluation = barretenberg::fr::serialize_from_buffer(&transcript.get_element("sigma_" + std::to_string(i + 1))[0]);
