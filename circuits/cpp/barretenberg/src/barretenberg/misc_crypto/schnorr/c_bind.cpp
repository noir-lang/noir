...
WASM_EXPORT void construct_signature(uint8_t const* message, size_t msg_len, uint8_t const* private_key, uint8_t* s, uint8_t* e)
{
    auto priv_key = grumpkin::fr::serialize_from_buffer(private_key);
    grumpkin::g1::affine_element pub_key = grumpkin::g1::one * priv_key;
    crypto::schnorr::key_pair<grumpkin::fr, grumpkin::g1> key_pair = { priv_key, pub_key };
    auto sig = crypto::schnorr::construct_signature<Blake2sHasher, grumpkin::fq>(std::string((char*)message, msg_len), key_pair);
    std::copy(sig.s.begin(), sig.s.end(), s);
    std::copy(sig.e.begin(), sig.e.end(), e);
}

WASM_EXPORT bool verify_signature(uint8_t const* message,
                                  size_t msg_len,
