...
{
    field_ct in_index1_field = witness_ct(&ctx.composer, in_index1);
    field_ct in_index2_field = witness_ct(&ctx.composer, in_index2);
    ...
    field_ct total_input = field_ct(in_note1_data.first.value) + field_ct(in_note2_data.first.value);
    ...
    auto note1 = create_destroy_note_context(ctx, in_index1_field, in_note1_data, witness_ct(&ctx.composer, true));
    set_note_public(ctx.composer, note1.note_data.second);
    destroy_note(ctx, note1);

    auto note2 = create_destroy_note_context(ctx, in_index2_field, in_note2_data, witness_ct(&ctx.composer, true));
