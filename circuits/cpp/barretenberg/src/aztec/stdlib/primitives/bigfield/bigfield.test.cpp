...
typedef stdlib::bigfield<waffle::TurboComposer, barretenberg::Bn254FqParams> pbigfield;
typedef stdlib::witness_t<waffle::TurboComposer> witness_t;
typedef stdlib::witness_t<waffle::TurboComposer> pwitness_t;
...
TEST(stdlib_bigfield, test_dual_madd)
{
    waffle::TurboComposer composer = waffle::TurboComposer();
    size_t num_repetitions = 1;
    for (size_t i = 0; i < num_repetitions; ++i) {
        ...
        bigfield d(witness_t(&composer, barretenberg::fr(uint256_t(inputs[3]).slice(0, bigfield::NUM_LIMB_BITS * 2))),
                   witness_t(&composer,
                             barretenberg::fr(uint256_t(inputs[3]).slice(bigfield::NUM_LIMB_BITS * 2,
                                                                         bigfield::NUM_LIMB_BITS * 4))));
        bigfield e(witness_t(&composer, barretenberg::fr(uint256_t(inputs[4]).slice(0, bigfield::NUM_LIMB_BITS * 2))),
                   witness_t(&composer,
                             barretenberg::fr(uint256_t(inputs[4]).slice(bigfield::NUM_LIMB_BITS * 2,
                                                                         bigfield::NUM_LIMB_BITS * 4))));

        uint64_t before = composer.get_num_gates();
        typename bigfield::cached_product temp;
        bigfield f = bigfield::dual_madd(a, b, c, d, { e }, temp);
        uint64_t after = composer.get_num_gates();
        if (i == num_repetitions - 1) {
            std::cout << "num gates per mul = " << after - before << std::endl;
        }
...
TEST(stdlib_bigfield_turbo, test_mul)
{
    waffle::TurboComposer composer = waffle::TurboComposer();
    size_t num_repetitions = 1;
    for (size_t i = 0; i < num_repetitions; ++i) {
...
TEST(stdlib_bigfield_turbo, test_sqr)
{
    waffle::TurboComposer composer = waffle::TurboComposer();
    size_t num_repetitions = 10;
    for (size_t i = 0; i < num_repetitions; ++i) {
