...
template <typename ComposerContext> class field_t {
  public:
     ...
     * i.e. if we want to multiply a wire by a constant, or add a constant, we do not need to add extra gates to do this.
     * Instead, we track the scaling factors, and apply them to the relevant wires when adding constraints
     * 
     * This also makes constant field_t objects effectively free. Where 'constant' is a circuit constant, not a C++ constant!
    ...
    /**
     * Every composer object contains a list of 'witnesses', circuit variables that can be assigned to wires when creating constraints
     * `witness_index` describes a location in this container. i.e. it 'points' to a circuit variable
     * 
     * A witness is not the same thing as a 'wire' in a circuit. Multiple wires can be assigned to the same witness via Plonk's copy constraints.
     * Alternatively, a witness might not be assigned to any wires! This case would be similar to an unused variable in a regular program
     * 
     * e.g. if we write `field_t foo = witness_t(context, 100)`, this will add the value `100` into `context`'s list of circuit variables.
     * However if we do not use `foo` in any operations, then this value will never be assigned to a wire in a circuit
     * 
     ...
     * 
     * This will add 3 new circuit witnesses (10, 50, 570). One constraint will also be created, that validates `baz` has been correctly constructed
     * The composer will assign `foo, bar, baz` to wires `w_1, w_2, w_3` in a gate, and check that:
     * 
     *      w_1 * w_2 + w_1 * 7 - w_3 = 0
     * 
     ...
     * 
     * The `w` value are wires, the `q` values are selector constants. If a field object contains a witness index, it will
     * be assigned to `w` values when constraints are applied. If it's a circuit constant, it will be assigned to `q` values
     * 
