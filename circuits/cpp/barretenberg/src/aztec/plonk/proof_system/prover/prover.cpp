...
template <typename settings> void ProverBase<settings>::execute_sixth_round()
{
    ...
    //             transcript.get_challenge_from_map("nu", "w_" + std::to_string(i + 1) + "_omega").begin()));
    //     }
    //     nu_challenges.emplace_back(
    //         fr::serialize_from_buffer(transcript.get_challenge_from_map("nu", "w_" + std::to_string(i +
    //         1)).begin()));
    // }
    ...
    //     z_powers[i] = zeta.pow(static_cast<uint64_t>(n * i));
    // }
