                                                                 ...
                                                                 const bool use_linearisation)
{
    ...
    fr new_nu_base = ProverTurboArithmeticWidget::compute_opening_poly_contribution(
        nu_base, transcript, poly, shifted_poly, use_linearisation);
...
barretenberg::fr VerifierTurboFixedBaseWidget::compute_quotient_evaluation_contribution(
    ...
    const bool use_linearisation)
{
    fr new_alpha_base = VerifierTurboArithmeticWidget::compute_quotient_evaluation_contribution(
    ...
    if (use_linearisation) {
        ...
        fr x_alpha_identity = -(w_3_omega_eval * alpha_b);

        fr T0 = w_1_omega_eval + w_1_eval + w_3_omega_eval;
        ...
        fr x_init_identity = -(w_1_eval * w_3_eval) * alpha_f;
        ...
        fr q_4_eval = fr::serialize_from_buffer(&transcript.get_element("q_4")[0]);
        fr q_5_eval = fr::serialize_from_buffer(&transcript.get_element("q_5")[0]);
        fr q_m_eval = fr::serialize_from_buffer(&transcript.get_element("q_m")[0]);

        constexpr fr minus_nine = -fr(9);
        constexpr fr minus_one = -fr(1);

        fr accumulator_delta = w_4_eval + w_4_eval;
        accumulator_delta += accumulator_delta;
        ...
        // => q_3 = (3.y_beta - y_gamma) / 3.(x_beta - x_gamma)
        // => q_ecc_1 = (3.x_beta.y_gamma - x_gammay_beta) / 3.(x_beta - x_gammma)
        fr y_alpha = w_3_omega_eval * q_3_eval;
        ...
        // x_alpha is the x-coordinate of the point we're adding into our accumulator point.
        // We use a w_o(X) to track x_alpha, to reduce the number of required selector polynomials
        fr x_alpha_identity = accumulator_delta_squared * q_1_eval;
        ...
        // ((x_2 + x_1 + x_alpha)(x_alpha - x_1)^2 - (y_alpha - y_1)^2).q_ecc = 0 mod Z_H
        // we use the fact that y_alpha^2 = x_alpha^3 + grumpkin::g1::element::curve_b
        fr x_alpha_minus_x_1 = w_3_omega_eval - (w_1_eval);

        T0 = y_alpha * w_2_eval;
        T0 += T0;

        T1 = x_alpha_minus_x_1.sqr();
        fr T2 = w_1_omega_eval + w_1_eval; // T1 = (x_alpha - x_1)^2
        ...
        // We constrain a_1 to be either 0 or the value in w_o (which should be correctly initialized to (1 / 4^n) via a
        // copy constraint) We constraint (x_1, y_1) to be one of 4^n.[1] or (4^n + 1).[1]
        fr w_4_minus_one = w_4_eval + minus_one;
