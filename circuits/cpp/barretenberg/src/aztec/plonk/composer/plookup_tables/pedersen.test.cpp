...
TEST(pedersen, validate_lookup_table_single)
{
    ...
    for (size_t i = num_entries - 1; i > 0; --i) {
        grumpkin::g1::element target(waffle::pedersen_tables::get_generator_value(generator_index, i, wnaf_entries[i]));
        const uint64_t wnaf_entry = wnaf_entries[i];
        const uint64_t negative = (wnaf_entry >> 31);
...
TEST(pedersen, validate_lookup_table)
{
    ...
    for (size_t i = 1; i < num_entries; ++i) {
        grumpkin::g1::element to_add(waffle::pedersen_tables::get_generator_value(generator_index, i, wnaf_entries[i]));
