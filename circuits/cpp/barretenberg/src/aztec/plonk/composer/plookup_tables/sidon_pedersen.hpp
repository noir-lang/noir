...
// namespace sidon_pedersen_tables {
...
// std::vector<uint64_t> compute_prime_factors(uint64_t input)
// {
...
// template <uint64_t sidon_prime> using sidon_fq = barretenberg::field<SidonFqParams<sidon_prime>>;

// template <uint64_t sidon_prime>
// using sidon_fq2 = barretenberg::field2<sidon_fq<sidon_prime>, SidonFq2Params<sidon_fq<sidon_prime>>>;

// template <uint64_t set_size> inline sidon_fq2<compute_nearest_safe_prime(set_size)> get_sidon_generator()
// {
//     constexpr uint64_t q = compute_nearest_safe_prime(set_size);

//     typedef barretenberg::field<SidonFqParams<q>> fq;
//     typedef barretenberg::field2<fq, SidonFq2Params<fq>> fq2;
...
//  *
//  * The technique can be extended to find Sidon sequences that are greater than 2 (e.g. sets where all combinations of
//  3 *set members are unique) This can be done by using a higher degree field extension - currently not implemented
//  **/
// template <uint64_t set_size> std::vector<uint64_t> compute_sidon_set()
// {
//     constexpr uint64_t q = compute_nearest_safe_prime(set_size);
...
// constexpr size_t TABLE_SIZE = (1UL) << BITS_PER_TABLE;
// constexpr size_t TABLE_MULTIPLICITY = 3; // using our sidon sequences, we can read from the same table three times
// constexpr size_t NUM_PEDERSEN_TABLES = (BITS_PER_HASH + (BITS_PER_TABLE * TABLE_MULTIPLICITY)) / (BITS_PER_TABLE *
// TABLE_MULTIPLICITY);

// static std::vector<uint64_t> sidon_set;

// static std::array<std::vector<grumpkin::g1::element>,
// std::vector<grumpkin::g1::element> compute_sidon_lookup_table(const grumpkin::g1::element generator)
// {
//     std::vector<grumpkin::g1::affine_element> table;
//     for (size_t i = 0; i < TABLE_SIZE; ++i)
//     {
//         table.emplace_back(generator * sidon_set[i]);
//     }
//     return table;
// }

// init_sidon_lookup_tables()
// {
