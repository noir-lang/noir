...
TEST(reference_string, mem_file_consistency)
{
    ...
    auto mem_crs = std::make_unique<waffle::MemReferenceStringFactory>((char*)monomials.data(), monomials.size(), (char*)g2x.data());
    ...
    EXPECT_EQ(memcmp(mem_verifier->get_precomputed_g2_lines(), file_verifier->get_precomputed_g2_lines(), sizeof(barretenberg::pairing::miller_lines) * 2), 0);
}