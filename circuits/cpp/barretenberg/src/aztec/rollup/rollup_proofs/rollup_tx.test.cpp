...
TEST(rollup_tx, test_serialization)
{
    ...
    rollup.old_data_paths =
        std::vector(rollup.num_txs * 2, std::make_pair(uint128_t(0), fr_hash_path(32, random_pair)));
    rollup.old_null_paths =
        std::vector(rollup.num_txs * 2, std::make_pair(uint128_t(0), fr_hash_path(128, random_pair)));
