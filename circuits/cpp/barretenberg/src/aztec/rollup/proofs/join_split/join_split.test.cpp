...
class join_split_tests : public ::testing::Test {
  protected:
    ...
    virtual void SetUp()
    {
        ...
        const uint32_t virtual_asset_id = (uint32_t(1) << 31) + defi_interaction_nonce;
...
TEST_F(join_split_tests, test_defi_non_zero_public_value_fails)
{
    ...
    bridge_id bridge_id = { 0, tx.asset_id, 0, 0, 0, true, false };
...
TEST_F(join_split_tests, test_repayment_logic)
{
    ...
    bridge_id bridge_id = { 0, tx.asset_id, 0, 0, defi_interaction_nonce, false, true };
    tx.claim_note.bridge_id = bridge_id.to_uint256_t();
...
TEST_F(join_split_tests, test_repayment_incorrect_nonce_fails)
{
    ...
    bridge_id bridge_id = { 0, tx.asset_id, 0, 0, defi_interaction_nonce + 1, false, true };
    tx.claim_note.bridge_id = bridge_id.to_uint256_t();
...
HEAVY_TEST_F(join_split_tests, test_repayment_full_proof)
{
    ...
    auto partial_commitment =
        value::create_partial_commitment(tx.claim_note.note_secret, tx.input_note[0].owner, tx.input_note[0].nonce, 0);
    claim::claim_note claim_note = { tx.claim_note.deposit_value, tx.claim_note.bridge_id,      0, 0,
                                     partial_commitment,          tx.claim_note.input_nullifier };
