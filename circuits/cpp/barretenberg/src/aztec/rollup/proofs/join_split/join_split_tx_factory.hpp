...
template <typename WorldState> class JoinSplitTxFactory {
  public:
    ...
    auto create_join_split_tx(std::array<uint32_t, 2> in_note_idx,
                              ...
                              uint32_t nonce = 0)
    {
        ...
        auto note_secret = user.note_secret;
        value_note input_note1 = { in_note_value[0], asset_id, nonce, sender, note_secret };
        value_note input_note2 = { in_note_value[1], asset_id, nonce, sender, note_secret };
        value_note output_note1 = { out_note_value[0], asset_id, nonce, receiver, note_secret };
        value_note output_note2 = { out_note_value[1], asset_id, nonce, sender, note_secret };
    ...
    auto create_defi_deposit_tx(std::array<uint32_t, 2> in_note_idx,
                                std::array<uint32_t, 2> in_note_value,
                                std::array<uint32_t, 2> out_note_value,
                                uint256_t bridge_id,
                                uint32_t asset_id = 0)
