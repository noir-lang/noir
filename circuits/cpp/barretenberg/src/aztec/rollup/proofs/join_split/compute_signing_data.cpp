...
barretenberg::fr compute_signing_data(join_split_tx const& tx)
{
    ...
    claim::claim_note claim_note = { tx.claim_note.deposit_value, tx.claim_note.bridge_id, 0, 0, partial_value_note_commitment, tx.claim_note.input_nullifier };
    ...
    std::vector<grumpkin::fq> to_compress{ public_value,
                                           tx.public_owner,
                                           grumpkin::fq(public_asset_id),
                                           output_note_1,
                                           output_note_2,
                                           nullifier1,
                                           nullifier2,
                                           tx.propagated_input_index,
                                           tx.backward_link,
                                           tx.allow_chain };
