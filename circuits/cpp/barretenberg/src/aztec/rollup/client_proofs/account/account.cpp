...
void account_circuit(Composer& composer, account_tx const& tx)
{
    ...
    const point_ct owner_pub_key(
        { witness_ct(&composer, tx.owner_pub_key.x), witness_ct(&composer, tx.owner_pub_key.y) });
    const uint32_ct num_new_keys = witness_ct(&composer, tx.num_new_keys);
    const auto new_account_note_1 = notes::account_note(owner_pub_key, tx.new_signing_pub_key_1, num_new_keys >= 1);
    const auto new_account_note_2 = notes::account_note(owner_pub_key, tx.new_signing_pub_key_2, num_new_keys >= 2);
    const auto remove_account =
        notes::account_note(owner_pub_key, tx.nullified_key, bool_ct(&composer, tx.nullify_key));
    const field_ct alias = witness_ct(&composer, tx.alias);
    const auto signing_account_note = notes::account_note(owner_pub_key, tx.signing_pub_key, true);
