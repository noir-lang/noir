...
class client_proofs_account : public ::testing::Test {
  protected:
...
TEST_F(client_proofs_account, test_create_account)
{
    auto tx = create_account_tx();
    EXPECT_TRUE(verify_logic(tx));
}

HEAVY_TEST_F(client_proofs_account, test_create_account_full_proof)
{
    auto tx = create_account_tx();
    EXPECT_TRUE(verify(tx));
}

TEST_F(client_proofs_account, test_missing_signing_key_fails)
{
    auto tx = create_account_tx();
...
TEST_F(client_proofs_account, test_alternative_signing_key_1)
{
...
TEST_F(client_proofs_account, test_alternative_signing_key_2)
{
    preload_account_notes();
    auto tx = create_account_tx();
    tx.account_index = 1;
...
HEAVY_TEST_F(client_proofs_account, test_correct_alias_nullifier)
{
    auto tx = create_account_tx();
    auto prover = new_account_prover(tx);
    auto proof = prover.construct_proof();
...
HEAVY_TEST_F(client_proofs_account, test_correct_account_nullifier)
{
    auto tx = create_account_tx();
    auto prover = new_account_prover(tx);
    auto proof = prover.construct_proof();
