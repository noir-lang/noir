...
typedef proof_system::TurboCircuitConstructor Composer;
typedef stdlib::byte_array<Composer> byte_array;
typedef stdlib::byte_array<proof_system::UltraCircuitConstructor> byte_array_plookup;
typedef stdlib::public_witness_t<Composer> public_witness_t;
typedef stdlib::public_witness_t<proof_system::UltraCircuitConstructor> public_witness_t_plookup;
...
{
    proof_system::UltraCircuitConstructor composer = proof_system::UltraCircuitConstructor();
    std::string input = "abcdefghijklmnopqrstuvwxyz0123456789abcdefghijklmnopqrstuvwxyz01";
    std::vector<uint8_t> input_v(input.begin(), input.end());

    byte_array_plookup input_arr(&composer, input_v);
    byte_array_plookup output = stdlib::blake3s<proof_system::UltraCircuitConstructor>(input_arr);
