...
TEST_F(state_var_tests, mapping)
{
    ...
    Mapping<Field> my_mapping(&exec_ctx, "balances", start_slot);
...
TEST_F(state_var_tests, mapping_within_mapping)
{
    ...
    Mapping<Mapping<Field>> my_mapping(&exec_ctx, "balances", start_slot);
...
TEST_F(state_var_tests, utxo_of_default_private_note_fr)
{
    ...
    UTXO<Note> my_utxo(&exec_ctx,
                       "balance",
                       start_slot); // TODO: handshake with exec_ctx to get the start slot, rather than passing one in.
    ...
    CT::fr new_value = *(old_note.get_preimage().value) + 5;

    info("new_value: ", new_value);

    // my_utxo.insert({ .value = new_value });

    // storage_slot, { value, owner }, salt, nonce
}
