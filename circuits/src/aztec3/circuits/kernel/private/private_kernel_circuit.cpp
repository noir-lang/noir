...
void ensure_consistency_between_kernels(PrivateInputs<CT> const& private_inputs, PublicInputs<CT>& public_inputs)
{
    ...
    end.callback_stack = start.callback_stack;
    end.optionally_revealed_data = start.optionally_revealed_data;
    end.output_commitments = start.output_commitments;
    end.input_nullifiers = start.input_nullifiers;
}

void validate_inputs(PrivateInputs<CT> const& private_inputs, PublicInputs<CT>& public_inputs)
{
...

    {
        ...
        is_base_case.must_imply(private_inputs.private_call.call_stack_item.is_delegate_call == false &&
                                    private_inputs.private_call.call_stack_item.is_static_call == false,
    ...
    ensure_consistency_between_kernels(private_inputs, public_inputs);
    validate_private_call_hash(private_inputs);

    // TODO: need to make this assertion somewhere, or only have call_context exist in one of these places (it _has_ to
    // be in the private_inputs.private_call.call_stack_item.public_inputs... and actually, when making a call, I think
    // it has to be in the call_stack_item, since the public inputs of the thing being called aren't known yet... so it
    // has to be in both places!!!)!

    (private_inputs.private_call.call_stack_item.call_context ==
     private_inputs.private_call.call_stack_item.public_inputs.call_context)
        .assert_equal(true, "Call context mismatches");
}
...
void update_end_values(PrivateInputs<CT> const& private_inputs, PublicInputs<CT>& public_inputs)
{
    ...
    const auto& is_static_call = private_inputs.private_call.call_stack_item.is_static_call;
    ...
    const auto& storage_contract_address =
        private_inputs.private_call.call_stack_item.call_context.storage_contract_address;
...

    {
        ...
        // in the stack can be validated as being correct. (e.g. call_contexts of calls made by the public_call
...
void private_kernel_circuit(Composer& composer, OracleWrapper& oracle, PrivateInputs<NT> const& _private_inputs)
{
    ...
    validate_inputs(private_inputs, public_inputs);
