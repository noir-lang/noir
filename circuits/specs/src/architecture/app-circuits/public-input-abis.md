...
| -------- | -------- |
| `customPublicInputs` | Up to 32 circuit-specific inputs passed into this circuit. All of these inputs will be 'swallowed' by the private kernel snark. <br/> Note: for a private circuit, we don't need a field for 'custom outputs' from the circuit, since all inputs/output must be known in advance, and so for simplicity can all be considered 'inputs'. |
