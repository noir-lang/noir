...
struct Context {
    ...
    /// Maps SSA values to array addresses
    ///
    /// When converting parameters the of main, `ArrayId`s are gathered and stored.
    ssa_value_to_array_address: HashMap<ValueId, ArrayId>,
        ...
        for param_id in params {
            ...
            match param_type {
                ...
                Type::Reference => {
                    ...
                    self.ssa_value_to_array_address.insert(*param_id, array_id);
    ...
    fn convert_ssa_instruction(&mut self, instruction_id: InstructionId, dfg: &DataFlowGraph) {
        ...
        let (results_id, results_vars) = match instruction {
            ...
            Instruction::Load { address, offset } => {
                let result_acir_var = self.convert_ssa_load(address, offset, dfg);
                let result_ids = dfg.instruction_results(instruction_id);
                (vec![result_ids[0]], vec![result_acir_var])
            }
