...
pub fn legacy_create_circuit(
    ...
    let all_parameters: BTreeSet<Witness> = param_witnesses.values().flatten().copied().collect();
    let private_parameters = all_parameters.difference(&public_parameters).copied().collect();
