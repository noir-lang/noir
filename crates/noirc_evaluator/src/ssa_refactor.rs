...
pub fn experimental_create_circuit(
    ...
    let public_parameters =
        abi.param_witnesses.values().flatten().copied().collect::<BTreeSet<_>>();
    let public_parameters = PublicInputs(public_parameters);
    let return_values = return_witnesses.into_iter().collect::<BTreeSet<_>>();
    let return_values = PublicInputs(return_values);
