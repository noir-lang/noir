...
impl Barretenberg {
    pub fn compress_native(&mut self, left: &Scalar, right: &Scalar) -> Scalar {
        ...
        Scalar::from_bytes(&result_bytes)
    }
    pub fn compress_many(&mut self, inputs: Vec<Scalar>) -> Scalar {
...
fn basic_interop() {
    ...
    for test in tests {
        let expected = Scalar::from_hex(test.expected_hex).unwrap();
