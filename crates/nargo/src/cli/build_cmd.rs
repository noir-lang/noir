...
mod tests {
    ...
    fn pass() {
        ...
        for path in paths.flatten() {
            let path = path.path();
            assert!(super::build_from_path(path.clone(), false).is_ok(), "path: {}", path.display());
        }
    }
    ...
    fn fail() {
        ...
        for path in paths.flatten() {
            let path = path.path();
            assert!(super::build_from_path(path.clone(), false).is_err(), "path: {}", path.display());
        }
    }
