...
pub fn execute_circuit(
    ...
    match solver_status {
        PartialWitnessGeneratorStatus::RequiresOracleData { 
            ...
            if required_oracle_data.len() != 0 {
                unreachable!("oracles are not supported by nargo execute")
            }
            for unresolved_brillig_call in unresolved_brillig_calls {
                let UnresolvedBrilligCall { foreign_call_wait_info, mut brillig } = unresolved_brillig_call;
                ...
                let solver_status = solve(backend, &mut initial_witness, &mut blocks, next_opcodes_for_solving)?;
