...
/// TODO almost identical as schnorr::construct_public_key...TODO TO REMOVE
pub fn fixed_base(input: &[u8; 32]) -> ([u8;32],[u8;32]) {
    ...
    (*slice_as_array!(&result[0..32], [u8; 32]).unwrap(),  *slice_as_array!(&result[32..64], [u8; 32]).unwrap())
...
pub fn construct_signature(message: &[u8], private_key: [u8; 32]) -> ([u8; 32], [u8; 32]) {
    ...
    let mut e = [0_u8; 32];
    unsafe{
        schnorr::construct_signature(message.as_ptr() as *const u8, message.len() as u64, private_key.as_ptr() as *const u8, 
            s.as_mut_ptr(), e.as_mut_ptr());
    }
...
//n.b. sig: [u8; 64],
pub fn verify_signature(pub_key: [u8; 64], sig_s: [u8; 32], sig_e: [u8; 32], message: &[u8]) -> bool {
    let mut r = false;
    unsafe{
        r = schnorr::verify_signature(message.as_ptr() as *const u8, message.len() as u64, 
            pub_key.as_ptr() as *const u8, sig_s.as_ptr() as *const u8, sig_e.as_ptr() as *const u8 );
    }
    r
