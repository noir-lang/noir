...
/// Generates artifacts for recursive verification of circuits
...
pub(crate) fn run<B: Backend>(
    backend: &B,
    args: RecursionCommand,
    config: NargoConfig,
) -> Result<(), CliError<B>> {
    let proof_path =
        config.program_dir.join(PROOFS_DIR).join(&args.proof_name).with_extension(PROOF_EXT);
    ...
    gen_recursion_artifacts_with_path(
        backend,
        config.program_dir,
        proof_path,
        circuit_build_path,
        recursion_artifacts_path,
        args.circuit_name,
    )?;
...
pub(crate) fn gen_recursion_artifacts_with_path<B: Backend, P: AsRef<Path>>(
    backend: &B,
    program_dir: P,
    proof_path: PathBuf,
    ...
    let common_reference_string =
        get_common_reference_string(backend, &preprocessed_program.bytecode)
    ...
    let (vk_fields, vk_hash) =
        vk_as_fields(backend, &common_reference_string, &preprocessed_program.verification_key)
            .map_err(CliError::ProofSystemCompilerError)?;
