...
mod tests {
    ...
    fn execution_success(
        ...
        write_test_artifact(&test_program_dir, force_brillig, inliner);
    }
    ...
    fn write_test_artifact(
        ...
        let dir_name = test_program_dir.file_name().unwrap().to_string_lossy().to_string();
        let artifact_filename = test_program_dir.join("target").join(format!("{}.json", dir_name));

        if !artifact_filename.exists() {
            return;
        }
        ...
        // We store the artifact for this matrix run in a directory that's not ignored in .gitignore
        let test_target_dir = test_program_dir.join("test_target");
        let _ = fs::create_dir(&test_target_dir);
        let matrix_artifact_name = test_target_dir.join(format!(
            "{}_force_brillig_{}_inliner_{}.json",
            dir_name, force_brillig.0, inliner.0
        ));
        let matrix_artifact_file = fs::File::create(matrix_artifact_name).unwrap();
        let matrix_artifact_file = BufWriter::new(matrix_artifact_file);
        serde_json::to_writer(matrix_artifact_file, &artifact).unwrap();
    }
