...
mod tests {
    ...
    #[test]
    fn rejects_unexpected_prover_input() {
        let project_dir = tempfile::tempdir().unwrap();
        fs::create_dir(project_dir.path().join("src")).unwrap();
        fs::write(
            project_dir.path().join("Nargo.toml"),
            r#"[package]
name = "unexpected_prover_input"
type = "bin"
authors = []
...
"#,
        ...
        .unwrap();
        fs::write(
            project_dir.path().join("src/main.nr"),
            "fn main(input: Field) { assert(input == 1); }\n",
        )
        .unwrap();
        fs::write(project_dir.path().join("Prover.toml"), "input = \"1\"\nextra = \"2\"\n")
            .unwrap();

        #[allow(deprecated)]
        let mut cmd = Command::cargo_bin("nargo").unwrap();
        cmd.arg("--program-dir").arg(project_dir.path()).arg("execute");
        cmd.assert().failure().stderr(
            predicate::str::contains("Received input arguments not expected by ABI")
                .and(predicate::str::contains("extra"))
