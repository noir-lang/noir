...
pub(crate) struct GenerateCompletionScriptCommand {
    /// The shell to generate completions for. One of: bash, elvish, fish, powershell, shell, zsh
    pub(crate) shell: String,
}

pub(crate) fn run(command: GenerateCompletionScriptCommand) -> Result<(), CliError> {
    let shell =
        match command.shell.to_lowercase().as_str() {
            "bash" => Shell::Bash,
            "elvish" => Shell::Elvish,
            "fish" => Shell::Fish,
            "powershell" => Shell::PowerShell,
            "zsh" => Shell::Zsh,
            _ => return Err(CliError::Generic(
                "Invalid shell. Supported shells are: bash, elvish, fish, powershell, shell, zsh"
                    .to_string(),
            )),
        };
