...
pub fn type_depth(typ: &Type) -> usize {
    match typ {
        ...
        Type::Array(_, typ) | Type::List(typ) => 1 + type_depth(typ),
...
/// Check if the type is a list or an array.
pub(crate) fn is_array_or_list(typ: &Type) -> bool {
    matches!(typ, Type::Array(_, _) | Type::List(_))
}
...
pub fn contains_reference(typ: &Type) -> bool {
    match typ {
        ...
        Type::Array(_, typ) | Type::List(typ) => contains_reference(typ),
        Type::Tuple(types) => types.iter().any(contains_reference),
    }
}

/// Check if the type contains any references.
pub fn contains_list(typ: &Type) -> bool {
    match typ {
        Type::List(_) => true,
        Type::Field
        | Type::Integer(_, _)
        ...
        | Type::FmtString(_, _)
        | Type::Function(_, _, _, _) => false,
        Type::Array(_, typ) | Type::Reference(typ, _) => contains_list(typ),
        Type::Tuple(types) => types.iter().any(contains_list),
    }
}
...
pub fn is_printable(typ: &Type) -> bool {
    match typ {
        ...
        Type::List(typ) | Type::Array(_, typ) | Type::FmtString(_, typ) => is_printable(typ),
...
pub fn to_hir_type(typ: &Type) -> noirc_frontend::Type {
    ...
    match typ {
        ...
        Type::List(typ) => HirType::List(Box::new(to_hir_type(typ))),
