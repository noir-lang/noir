...
fn arb_program_can_be_executed() {
    let maybe_seed = common::seed_from_env();
    ...
    common::run_fuzz(
        |u| {
            ...
            let ssa = ssa::create_program(program.clone(), &options, None).unwrap_or_else(|e| {
                print_ast_and_panic(&format!("Failed to compile program: {e}"))
            });
