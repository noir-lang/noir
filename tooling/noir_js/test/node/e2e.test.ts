...
import { Noir as NoirProgram, generateWitness } from '@noir-lang/noir_js';
...
it('end-to-end proof creation and verification (outer)', async () => {
  ...
  const backend = new BarretenbergBackend(assert_lt_json);

  const program = new NoirProgram(assert_lt_json, backend);
...
it('[BUG] -- bb.js null function or function signature mismatch (different instance) ', async () => {
  ...
  try {
    const verifier = new BarretenbergBackend(assert_lt_json);
