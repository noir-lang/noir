...
fn type_contains_list_or_reference(type_: &Type) -> bool {
    match type_ {
        Type::List(_) => true,
        Type::Reference(_) => true,
        Type::Array(arr, _) => arr.iter().any(type_contains_list_or_reference),
...
pub(crate) fn fuzz_target(
    ...
    if type_contains_list_or_reference(&data.functions[0].return_type) {
