...
mod tests {
    ...
    fn build_nargo() -> PathBuf {
        let target_dir = std::env!("CARGO_MANIFEST_DIR");
        PathBuf::from(target_dir).join("../target/debug/nargo")
    }
    ...
    fn snapshot_output(output: Output, test_program_dir: &Path) {
        ...
        insta::with_settings!(
            {
                snapshot_path => test_program_dir,
