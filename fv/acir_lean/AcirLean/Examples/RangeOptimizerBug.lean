...
def buggyShippedDiv8 : AcirFn :=
  { shippedDivT 8 with cs := (shippedDivT 8).cs.filter (· != .range 1 8) }
...
theorem forgedDiv_satisfies : AllSat forgedDiv buggyShippedDiv8.cs := by decide +kernel

theorem buggy_range_optimizer_not_sound :
    ¬ SoundFn buggyShippedDiv8 (Computes2 8 (BinOp.eval .div)) := by
  intro h
  have hs := h forgedDiv forgedDiv_satisfies
  simp only [buggyShippedDiv8, shippedDivT, acirDivT, List.map, Computes2] at hs
