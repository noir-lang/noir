...
theorem divVarGadget_sound {n : ℕ} (hn : n ≤ 126) (σ : ℕ → F) (h : AllHold σ (divVarGadget n))
  ...
  have e1 := g (.assertZero [⟨1, []⟩, ⟨-1, [1, 2]⟩]) (by simp [divVarGadget])
  have r3 := g (.range 3 n) (by simp [divVarGadget])
  have r4 := g (.range 4 n) (by simp [divVarGadget])
  have e2 := g (.assertZero [⟨1, []⟩, ⟨-1, [1]⟩, ⟨1, [4]⟩, ⟨1, [5]⟩]) (by simp [divVarGadget])
  have r5 := g (.range 5 n) (by simp [divVarGadget])
  have e3 := g (.assertZero [⟨1, [0]⟩, ⟨-1, [1, 3]⟩, ⟨-1, [4]⟩]) (by simp [divVarGadget])
...
theorem truncateGadget_sound {k : ℕ} (hk1 : 2 ≤ k) (hk : k ≤ 125) (σ : ℕ → F)
  ...
  have e1 := g (.assertZero [⟨1, [0]⟩, ⟨-(2 ^ k : ℕ), [1]⟩, ⟨-1, [2]⟩]) (by simp)
  have e2 := g (.assertZero [⟨(q0 k : ℤ), []⟩, ⟨-1, [1]⟩, ⟨-1, [3]⟩]) (by simp)
  have r3 := g (.range 3 (254 - k)) (by simp)
  have e3 := g (.assertZero [⟨1, []⟩, ⟨1, [1, 4]⟩, ⟨-(q0 k : ℤ), [4]⟩, ⟨-1, [5]⟩])
    (by simp)
  have e4 := g (.assertZero [⟨1, [1, 5]⟩, ⟨-(q0 k : ℤ), [5]⟩]) (by simp)
  have e5 := g (.assertZero [⟨1, [2, 5]⟩, ⟨(R' k : ℤ), [5]⟩, ⟨-1, [6]⟩]) (by simp)
  have r6 := g (if N' k = 0 then .assertZero [⟨1, [6]⟩] else .range 6 (N' k)) (by simp)
  simp only [Opcode.Holds, Term.eval, List.map, List.prod_cons, List.prod_nil,
