...
def key (P : Poly) : Constraint := (Constraint.zero (collect P)).canon

/-- `P = 0` follows from the circuit: it is trivial, or one of its constraints. -/
def holdsZ (cc : List Constraint) (P : Poly) : Bool :=
  match key P with
  | .zero [] => true
  | c => decide (c ∈ cc)
...
constraint `b (w - c) = 0`. -/
def constCands (cc : List Constraint) (w : ℕ) : List (ℕ × ℤ) :=
...
def rangeBounds (cc : List Constraint) (w : ℕ) : List ℕ :=
  cc.filterMap fun c => match c with
...
def wbound (cc : List Constraint) (w : ℕ) : Option (ℕ × ℕ) :=
...
def eqVia (cc : List Constraint) (P Q : Poly) : Bool :=
  holdsZ cc (psub P Q) ||
...
def zeroFlags (cc : List Constraint) (t : Poly) : List ℕ :=
...
def remLt (cc : List Constraint) (Xb : Poly) (b : Rep2) (r Mr : ℕ) : Bool :=
...
def euclid (cc : List Constraint) (a b : Rep2) : List (ℕ × ℕ) :=
...
def geFlags (cc : List Constraint) (a b : Rep2) (m : ℕ) : List ℕ :=
  (forms cc a.alts).flatMap fun Xa => (forms cc b.alts).flatMap fun Xb =>
    (solve2 cc fun q r =>
...
/-- `eq`: the flag `a - b = 0`. -/
def eqFlags (cc : List Constraint) (a b : Rep2) : List ℕ :=
  (forms cc a.alts).flatMap fun Xa => (forms cc b.alts).flatMap fun Xb =>
    zeroFlags cc (psub Xa Xb)

/-- A bounded witness equal to one of the polynomials, below `2^n`. -/
def checked (cc : List Constraint) (alts : List Poly) (n : ℕ) : Option (ℕ × ℕ) :=
  alts.findSome? fun P => (pbound cc P).filter fun (_, M) => M < 2 ^ n

def binRep (cc : List Constraint) (op : BinaryOp) (a b : Rep2) : Option Rep2 :=
...
below `d + p % 2^k`. -/
def truncOK (cc : List Constraint) (k q r : ℕ) : Bool :=
  match wbound cc q, wbound cc r with
  | some (_, Mq), some (_, Mr) =>
...
def truncRep (cc : List Constraint) (a : Rep2) (k : ℕ) : Rep2 :=
...
def eqHolds (cc : List Constraint) (a b : Rep2) : Bool :=
  (forms cc a.alts).any fun Xa => (forms cc b.alts).any fun Xb => eqVia cc Xa Xb

def rangeHolds (cc : List Constraint) (a : Rep2) (k : ℕ) : Bool :=
  decide (a.M < 2 ^ k) || (checked cc a.alts k).isSome

def step2 (cc : List Constraint) (reps : List (ℕ × Rep2)) : Instruction → Option (List (ℕ × Rep2))
  | .bin d op _ a b => do
    let ra ← opRep reps a
    let rb ← opRep reps b
...
def paramRep (cc : List Constraint) (w : ℕ) : ValueType → Option Rep2
...
def initReps (cc : List Constraint) : List (ℕ × ValueType) → List ℕ → Option (List (ℕ × Rep2))
  | (id, ty) :: ps, w :: ws => do
...
def retOK (cc : List Constraint) (reps : List (ℕ × Rep2)) (r : ℕ) (o : Operand) : Bool :=
  match opRep reps o with
  | some ro => (forms cc ro.alts).any fun X => eqVia cc (pvar r) X
  | none => false

def checkProg2 (P : Program) (C : AcirFunction) : Bool :=
