...
template <typename Builder> class EcdsaCircuit {
  public:
    ...
    static Builder generate(uint256_t public_inputs[])
    {
        ...
        crypto::ecdsa::key_pair<typename curve::fr, typename curve::g1> account;
        account.private_key = curve::fr::random_element();
        account.public_key = curve::g1::one * account.private_key;

        // UNCONSTRAINED: create a sig
        crypto::ecdsa::signature signature = crypto::ecdsa::
            construct_signature<Sha256Hasher, typename curve::fq, typename curve::fr, typename curve::g1>(
        ...
        stdlib::ecdsa::signature<Builder> sig{ typename curve::byte_array_ct(&builder, rr),
                                               typename curve::byte_array_ct(&builder, ss),
                                               stdlib::uint8<Builder>(&builder, vv) };

        // IN CIRCUIT: verify the signature
        typename curve::bool_ct signature_result = stdlib::ecdsa::verify_signature<Builder,
                                                                                   curve,
