...
TEST_F(AvmMiniBitwiseNegativeTestsFF, UndefinedOverFF)
{
    auto trace_builder = avm_trace::AvmMiniTraceBuilder();
    ...
    for (size_t i = 1; i < 4; i++) {
        trace.at(i).memTrace_m_tag = FF(6);
        trace.at(i).memTrace_m_in_tag = FF(6);
        trace.at(i).aluChip_alu_ff_tag = FF::one();
        trace.at(i).aluChip_alu_u8_tag = FF::zero();
        trace.at(i).avmMini_in_tag = FF(6);
    }
...
TEST_F(AvmMiniBitwiseNegativeTestsU32, BitwiseNot)
{
