...
inline const uint32_t NULLIFIER_NON_EXISTS_OFFSET =
    NULLIFIER_EXISTS_OFFSET + MAX_NULLIFIER_READ_REQUESTS_PER_CALL * READ_REQUEST_LENGTH;
inline const uint32_t PUBLIC_DATA_UPDATE_OFFSET =
    NULLIFIER_NON_EXISTS_OFFSET + MAX_NULLIFIER_NON_EXISTENT_READ_REQUESTS_PER_CALL * READ_REQUEST_LENGTH;
inline const uint32_t PUBLIC_DATA_READ_OFFSET =
    PUBLIC_DATA_UPDATE_OFFSET + MAX_PUBLIC_DATA_UPDATE_REQUESTS_PER_CALL * CONTRACT_STORAGE_UPDATE_REQUEST_LENGTH;
inline const uint32_t PUBLIC_CALLSTACK_OFFSET =
    PUBLIC_DATA_READ_OFFSET + MAX_PUBLIC_DATA_READS_PER_CALL * CONTRACT_STORAGE_READ_LENGTH;
inline const uint32_t NOTE_HASHES_OFFSET = PUBLIC_CALLSTACK_OFFSET + MAX_PUBLIC_CALL_STACK_LENGTH_PER_CALL;
inline const uint32_t NULLIFIERS_OFFSET = NOTE_HASHES_OFFSET + MAX_NEW_NOTE_HASHES_PER_CALL * NOTE_HASH_LENGTH;
inline const uint32_t L2_TO_L1_MSGS_OFFSET = NULLIFIERS_OFFSET + MAX_NEW_NULLIFIERS_PER_CALL * NULLIFIER_LENGTH;

// Two item gap
inline const uint32_t UNENCRYPTED_LOGS_OFFSET =
