...
function validateVariants(variants: ResetVariant[]) {
  for (const variant of variants) {
    ...
    if (variant.replacements.NULLIFIER_KEYS > MAX_NULLIFIER_KEY_VALIDATION_REQUESTS_PER_TX) {
      throw new Error(`NULLIFIER_KEYS must be less than ${MAX_NULLIFIER_KEY_VALIDATION_REQUESTS_PER_TX}`);
    }
  }
}
