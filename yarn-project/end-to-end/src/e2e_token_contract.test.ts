...
describe('e2e_token_contract', () => {
  ...
  beforeAll(async () => {
    ({ teardown, logger, wallets } = await setup(3));
    ...
    tokenSim = new TokenSimulator(
      asset,
      logger,
      wallets.map(w => w.getAddress()),
    );

    expect(await asset.methods.admin().simulate()).toBe(wallets[0].getAddress().toBigInt());
  ...
  describe('Access controlled functions', () => {
    it('Set admin', async () => {
      await asset.methods.set_admin(wallets[1].getAddress()).send().wait();
      expect(await asset.methods.admin().simulate()).toBe(wallets[1].getAddress().toBigInt());
    });

    it('Add minter as admin', async () => {
      await asset.withWallet(wallets[1]).methods.set_minter(wallets[1].getAddress(), true).send().wait();
      expect(await asset.methods.is_minter(wallets[1].getAddress()).simulate()).toBe(true);
    });

    it('Revoke minter as admin', async () => {
      await asset.withWallet(wallets[1]).methods.set_minter(wallets[1].getAddress(), false).send().wait();
      expect(await asset.methods.is_minter(wallets[1].getAddress()).simulate()).toBe(false);
    });

    describe('failure cases', () => {
      it('Set admin (not admin)', async () => {
        await expect(asset.methods.set_admin(wallets[0].getAddress()).prove()).rejects.toThrow(
          'Assertion failed: caller is not admin',
        );
      });
      it('Revoke minter not as admin', async () => {
        await expect(asset.methods.set_minter(wallets[0].getAddress(), false).prove()).rejects.toThrow(
          'Assertion failed: caller is not admin',
        );
      });
    });
    ...
    describe('Private', () => {
      ...
      describe('Mint flow', () => {
        ...
        it('redeem as recipient', async () => {
          ...
          const txClaim = asset.methods.redeem_shield(wallets[0].getAddress(), amount, secret).send();
          // docs:start:debug
    ...
    describe('public', () => {
      ...
      describe('failure cases', () => {
        ...
        it('transfer more than balance on behalf of other', async () => {
          ...
          expect(
            await wallets[0].lookupValidity(wallets[0].getAddress(), { caller: wallets[1].getAddress(), action }),
          ).toEqual({
        ...
        it('transfer on behalf of other, cancelled authwit', async () => {
          ...
          await wallets[0].cancelAuthWit({ caller: wallets[1].getAddress(), action }).send().wait();
        ...
        it('transfer on behalf of other, cancelled authwit, flow 2', async () => {
          ...
          await wallets[0].setPublicAuthWit({ caller: wallets[1].getAddress(), action }, false).send().wait();
        ...
        it('transfer on behalf of other, invalid spend_public_authwit on "from"', async () => {
          ...
          const txCancelledAuthwit = asset
            .withWallet(wallets[1])
            .methods.transfer_public(badAccount.address, wallets[1].getAddress(), 0, nonce)
            .send();
    ...
    describe('private', () => {
      ...
      describe('failure cases', () => {
        ...
        it('transfer on behalf of other, invalid spend_private_authwit on "from"', async () => {
          ...
          const txCancelledAuthwit = asset
            .withWallet(wallets[1])
            .methods.transfer(badAccount.address, wallets[1].getAddress(), 0, nonce)
            .send();
          await expect(txCancelledAuthwit.wait()).rejects.toThrow(
            "Assertion failed: Message not authorized by account 'result == IS_VALID_SELECTOR'",
          );
