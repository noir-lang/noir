...
# Installs cvc5 for the noirc_evaluator acir_context::soundness tests. The -gpl
# build is the one with CoCoA, which its finite-field theory needs.
[private]
install-cvc5:
    #!/usr/bin/env bash
    set -euo pipefail
    ...
    url="https://github.com/cvc5/cvc5/releases/download/cvc5-1.4.0/$asset"
    tmp=$(mktemp -d)
